...
- **工作區**: packages/shared, packages/crm, apps/server, apps/web
- **類型**: feat
- **檔案**: `packages/shared/src/train.ts`, `packages/shared/src/crm-types.ts`, `packages/crm/migrations/021_synthetic_contact.sql`(新), `packages/crm/migrations-pg/021_synthetic_contact.sql`(新), `packages/crm/src/mappers.ts`, `packages/crm/src/ports.ts`, `packages/crm/src/repos-prospect.ts`, `apps/server/src/train/persona.ts`, `apps/server/src/train/persona-gen.ts`(新), `apps/server/src/train/persona-gen.test.ts`(新), `apps/server/src/train/train-service.ts`, `apps/server/src/train/routes.ts`, `apps/server/src/train/train-selfserve.test.ts`(新), `apps/server/src/index.ts`, `apps/web/lib/api.ts`, `apps/web/components/train/PersonaPicker.tsx`, `apps/web/components/train/SyntheticPersonaCreator.tsx`(新), `apps/web/components/train/personaMeta.ts`(新), `apps/web/components/train/TrainWorkbench.tsx`, `apps/web/components/crm/ContactsTab.tsx`, `apps/web/components/crm/PersonaCard.tsx`, `apps/web/app/globals.css`
- **改了什麼**:
  - **契約（shared）**: `train.ts` 加 `TrainObjective{salesGoal?,meetingPurpose?}`、`NewTrainSession.objective?`、`PersonaFieldDraft`（九欄＝Contact persona 欄位＝server PERSONA_FIELDS）、`PersonaDraftResult{fields}`、`NewSyntheticPersona{companyId,fullName?,title?,persona?,autoDesign?,difficulty?,objective?}`、`CreateSyntheticResult{contactId}`；**`PersonaOption` 加必填 `unlocked:boolean`**（補契約洞：client isReady 須 `unlocked || (missing===0 && verified>0)`，否則 #1 未驗證草稿會誤判鎖住）。`crm-types.ts` 加 `Contact.isSynthetic?`、`ContactSummary.isSynthetic?`。
  - **資料層（crm）**: migration 021（sqlite＋pg）`contacts ADD COLUMN is_synthetic INTEGER NOT NULL DEFAULT 0`；mappers CONTACT_DEFS 加 `{col:"is_synthetic",key:"isSynthetic"}`；`SqliteContactRepository` 新增 (a) `applyAiDraft(orgId,id,patch,meta)`——單 tx 內查 `trustedFieldsOf` 跳過已受信任欄、寫欄值＋各欄 provenance（`filledBy='llm'`/`sourceType='ai_draft'`/`verified=0`、confidence≈0.5、**不 bump verified_status**、值與來源同 tx，守 §11）；(b) `setTrainingUnlocked(orgId,id,unlocked)`——純寫旗標（**不寫 provenance、不 bump verified_status**，供 #1 解鎖用，避免抬高真人 contact 的可信徽章）；`contacts.list()` SELECT＋`mapContactSummary` 補 `is_synthetic`（虛擬 badge 資料來源）。ports.ts 同步加 `AiDraftMeta`＋兩方法簽名。
  - **server**: `persona-gen.ts`（新）`draftPersonaForContact`（#1 推斷真人）／`designSyntheticPersona`（#4 設計虛構）＋`personaDraftToContactPatch`，Gemini structured output 產九欄 zh-TW。`persona.ts` `buildPersonaPrompt` 加 `BuildPersonaOptions{unlocked?,objective?}`：**trainingUnlocked=1 時放寬用「所有非空 persona 欄」**（trusted ∪ 未驗證有值），否則維持 trusted-only（§9）；objective 有值時注入「本次對練情境」zh-TW 段。`train-service.ts` 加 `draftPersona`（applyAiDraft 寫未驗證草稿＋`setTrainingUnlocked(true)`）、`createSynthetic`（`create` is_synthetic=1 contact＋human `update` 寫 persona＋trainingUnlocked=1）；`personas()` 補 `companyId`/`fullNameZh`/`unlocked`；`startSession` 傳 objective 進 buildPersonaPrompt。`routes.ts` 加 `POST /personas/:contactId/draft`、`POST /synthetic`（201）＋`parseObjective`/`parsePersona`＋sessions 收 objective。`index.ts` 把兩新 LLM 端點加進 rate-limit 掛載清單（與 decks/generate·image-jobs·research/enrich·train/sessions 一致）。
  - **web**: `api.ts` 加 `draftPersona`/`createSyntheticPersona`＋`startTrainSession` 收 objective（型別直接 import shared）。`PersonaPicker.tsx` isReady 改用 unlocked；頂部「真人／AI 虛擬人物」segmented（role=group＋aria-pressed）；真人未 ready 卡加「讓 AI 補齊」鈕（draftPersona→refetch）；啟動列加 objective 兩欄；`onCreated(contactId,{difficulty,objective})` 帶設定回 launch 預填。`SyntheticPersonaCreator.tsx`（新）公司選擇＋「讓 AI 決定/手動九欄」＋objective/難度→createSyntheticPersona，按鈕文案「建立並選取對象」。`personaMeta.ts`（新）抽 DIFFICULTY_META/FIELD_LABELS（解 PersonaPicker↔Creator 互 import 成環）。`TrainWorkbench.tsx` onStart 收 objective 傳 startTrainSession。CRM `ContactsTab`/`PersonaCard` 加「虛擬」badge。`globals.css` 加對應 class。
- **為什麼**: 使用者要求對練對象不必是真人、且不必進 CRM 內部設定太麻煩（#4），並讓 AI 一鍵補齊真人 persona（#1），面談目的每次可填。使用者拍板：#1 補齊後直接可對練（欄位留未驗證草稿只翻 unlock，守 §11）、#4 虛擬人物也進 CRM 附 badge、objective 每次對練。純 CRM 資料＋對練設定，不動 deck patch/approval/HUD（I1/I2/I3 不受影響）；AI 對真人臆測不升 verified（applyAiDraft verified=0＋setTrainingUnlocked 不 bump），虛擬角色由使用者創作故 human provenance 合法。
- **驗證**: 六視角對抗式審查（Workflow）11 raw→2 確認高信心，全數已修（draftPersona 不再 bump verified_status／兩端點補 rate limit／虛擬按鈕文案＋帶設定／模式切換 ARIA 合法化）；被對抗式驗證濾掉 4 誤報。重建 shared＋crm dist 後：`typecheck` crm/server/web 全 EXIT=0；`apps/server vitest` 50 檔 285 測全綠（新增 train-selfserve 7＋persona-gen 8）；`crm vitest` 8 檔 67 測全綠（含 is_synthetic 落庫/summary 斷言）。authz 跨 org 對 /draft·/synthetic 皆 not_found 且無副作用（測試斷言）。未 commit（硬規則 10）。

### 2026-07-24 17:59 | train 頁小修：#3 顯示中文名（對齊 CRM）＋#2「補齊」深連結到該主管＋readiness 欄位標籤補齊（未部署）
- **工作區**: packages/shared, apps/server, apps/web
- **類型**: fix
- **檔案**: `packages/shared/src/train.ts`, `apps/server/src/train/train-service.ts`, `apps/web/components/train/PersonaPicker.tsx`, `apps/web/app/[locale]/crm/[id]/page.tsx`, `apps/web/components/crm/CompanyDetailView.tsx`, `apps/web/components/crm/ContactsTab.tsx`
- **改了什麼**:
  - **#3 中文名**：`PersonaOption` 加 `fullNameZh?`（＋`companyId`）；train-service `personas()` 由 `ContactSummary` 帶出 `fullNameZh: c.fullNameZh`、`companyId: c.companyId`；PersonaPicker 顯示改 `displayName = p.fullNameZh ?? p.fullName`（頭像/名稱/啟動列/確認框），與 CRM ContactsTab（line 86 已 `c.fullNameZh ?? c.fullName`）對齊——修「train 頁英文名、CRM 中文名」不一致。
  - **#2 深連結**：PersonaPicker「補齊後可對練 →」由 `href="/crm"`（外層清單）改 UrlObject `{ pathname: `/crm/${companyId}`, query: { tab:"contacts", contact:contactId } }`；`/crm/[id]/page.tsx`（Server Component）多收 `searchParams` prop、await 後把 `initialTab`/`initialContactId` 傳給 `CompanyDetailView`（免 `useSearchParams`＋Suspense 邊界）；CompanyDetailView 初始 tab 由 `initialTab` 決定（驗證屬 TABS 才採用，否則 overview），並把 `initialContactId` 傳給 ContactsTab；ContactsTab 加 `initialContactId?` prop 作為 `selected` 初值→深連結進來自動展開該主管。
  - **readiness 標籤**：PersonaPicker `FIELD_LABELS` 改成對齊 server `PERSONA_FIELDS`（persona.ts 9 鍵：communicationStyle/commStyleNotes/personalityNotes/decisionStyle/knownPriorities/goalsKpis/hotButtons/painPoints/objectionsRaised）逐一給繁中標籤，避免「缺 X」badge 漏標露英文欄名（原表鍵 objections/decisionPower/seniority/title 與實際 missing 鍵對不上）。
- **為什麼**: 使用者回報 train persona 卡「補齊」連到 /crm 外層難調整、且顯示英文名與 CRM 中文名不一致。純顯示與導覽修正，不動 deck patch/approval/HUD、不動信任閘與 canTrain 邏輯，不違反 I1/I2/I3。（#1 AI 自動補齊 persona ＋ #4 train 頁自助建「AI 虛擬人物」為後續大功能，另凍藍圖確認後再做。）
