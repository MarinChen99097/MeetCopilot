...
- Cloud Run：`meetcopilot-server`（min=0/max=1/cpu=2/mem=4Gi/gen2/CloudSQL/WS 3600/session-affinity）、`meetcopilot-web`（min=0/max=2/cpu=1/mem=1Gi）、`meetcopilot-admin`（min=0/max=1/cpu=1/mem=1Gi；**2026-07-25 首次上線**，網址 https://meetcopilot-admin-54139295474.asia-east1.run.app）
- 影像：`asia-east1-docker.pkg.dev/ezpagesite/meetcopilot/{server,web,admin}`

**目前版本（2026-08-20 會中收音改雙聲道 speaker 分軌＋結束會議＋殭屍會議防護封閉）**：server **`meetcopilot-server-00030-ssn`**、web **`meetcopilot-web-00031-wxv`**（admin 維持 `00001-qf8` 不動）；commits `e7d6a05` feat(realtime)＋`8f351bc` docs，push origin main（`5bab897..8f351bc`）；build server `95526688-7b5b-4609-897a-1f69072f942b`／web `786efc9f-e04e-4dea-b2fc-dd99f55d9c5f` 皆 SUCCESS；**無 migration、無新必填 env**（`ready:true` 佐證 boot 未 crash）；**shared/server/web 皆重建**（動了 `packages/shared` 的協定常數）；server `services update --image` 保 env（部署後 env count **20**，DB_DRIVER／WEB_ORIGIN／GOOGLE_CLIENT_ID／WS_PUBLIC_BASE／ADMIN_ORIGIN 皆在，未吹）；web `run deploy` 帶 NEXT_PUBLIC_API_BASE。
> ⚠️ **本次有部署順序約束（新增的部署陷阱，日後同類改動照辦）**：**server 必須先上、web 後上**。新 web 會在 WS 握手帶 `channels=2` 並送**交錯 stereo** PCM，而舊 server 不認得該 param → fail-safe 當 mono 處理 → 交錯資料被當單聲道解讀＝**嚴重混疊噪音＋音訊時鐘跑兩倍快**（且是靜默失敗）。反向順序安全：新 server ＋ 舊 web＝舊 web 不送 param、新 server 落 mono、資料也確實是 mono。
> 冒煙全綠：server `/api/health` 200＋`/api/ready` `{"ready":true}`＋未帶 token `/api/train/personas` 401；web `/` 307＋`/zh-TW`·`/zh-TW/{copilot,studio,train,present/start,spend}` 全 200＋**`/pcm-worklet.js` 200 且內容確認為新版**（`joinCarry`×5、`THE ONE ALLOWED COPY` 註解、overshoot clamp 皆在＝證明不是快取舊檔）；server 開機 log 無 error。
> 內容＝雙聲道收音全鏈（麥克風 L／分頁 R 交錯 stereo，deinterleave 置於 `hub.pushAudio` 最上游使 chunker 以下維持 mono，speaker 由聲道決定並跳過 `inferSpeaker`，麥克風被拒自動退回 mono）＋「結束這場會議」接線（後端 `endMeeting` 前端從未呼叫過）＋**殭屍會議封閉**（WS 握手加 org-scoped meeting-status 閘，completed 拒連、跨 org 與不存在回應逐位元相同；close code 與 `channels` 協商收進 `packages/shared` 單一真相）＋效能（靜音軌不再空打 Gemini、分析窗改字元預算使 mono 不替 stereo 付費）＋mono 真下混與 resampler drift 修正。經 /code-review（18 agents，L20 第四度生效：confirmed 0 但 killed 中 9 項 refuted:false，採納 7 含一個 CRITICAL）＋/simplify（四角度→做 13 跳 4）＋獨立 fresh-context 最終回歸（typecheck 五 workspace、`npm test` 五連綠 crm 88／server 514、worklet 240 組態 2708 frames 逐位元組相同）。
> 📌 **本機開發注意**：`packages/shared/dist/` 需重建才會被 server vitest 吃到（走 node_modules symlink，吃 dist 不吃 src），且 `tsconfig.json` 有 `composite: true`——**incremental build 會靜默跳過，只看 `exit 0` 不足以證明它真的重建了**，必要時刪 `dist/`＋`tsbuildinfo` 做 clean rebuild。（Cloud Build 走 Docker 從 source 建，不受此影響。）

前版（2026-08-09 前端接縫掃蕩：Studio 按鈕＋37 項孤兒 class/舊色/對齊/隱形字/死 CSS）：web **`meetcopilot-web-00030-pjw`**（server 維持 `00029-tj8` 不動——純 web）；commits `1ab2e8d` fix(web)＋`8f9d2bb` docs；build web `7ce0c2c3-2670-41fc-bf98-4e52f437b558` SUCCESS；**無 migration/env/相依**；`run deploy` 帶 NEXT_PUBLIC_API_BASE；冒煙 zh-TW/studio/login/spend/settings-team 全 200。內容＝機械稽核（窮舉）＋44 次載入量測揪出的簇狀接縫：I1 鎖頁提示 1.03→16.16:1、DeckWizard 舊色換膚、.mc-card 成原語、CRM 8 分頁對齊歸零、邀請「逾期」誤標修正、PRICING__ 佔位/env 名外洩修正、死 CSS 46 條清理；parity 476/476、行為零變更。）前版（2026-08-01 晚：版型 prompt/schema 瘦身——生成一次過率 38%→100%、耗時中位 -83%）**：server **`meetcopilot-server-00029-tj8`**（web 維持 `00029-qwh` 不動——只動 slide-gen.ts 故只重建 server）；commit `da44130`；build `440c8a1c-65ad-4280-a9fc-0cbe8ebf18bb` SUCCESS；**無 migration/env/相依**；`services update --image` 保 env；冒煙 health 200＋ready true。內容＝prompt 去重（deck -38.7%/supplement -34.5%、事實紀律段保留）＋生成端 schema 收斂 24→20 property（TS 型別/渲染/pptx 不動）＋stats 頁條件化＋新版式選版指示（命中 57%→100%）；L22＝Gemini responseSchema min/maxItems 文法展開預算（超過整份 400）。真 API 16 連跑 A/B 實證。）前版（2026-08-01 上線首日修復：生成 RECITATION/MAX_TOKENS 韌性＋Studio 編輯器三修）：server **`meetcopilot-server-00028-tfg`**、web **`meetcopilot-web-00029-qwh`**；commits `a0282f1` fix(server)＋`0faa870` fix(web)＋`7e298d5` docs；build server `67b4df1b-d4e6-41fc-b399-eb6d0c1ec43b`／web `a3ad6f99-0897-43c6-be05-87530fea7224` 皆 SUCCESS；**無 migration、無新必填 env**；server `services update --image` 保 env、web `run deploy` 帶 NEXT_PUBLIC_API_BASE；冒煙 health 200＋ready true＋web zh-TW/studio/copilot/train 全 200。內容＝生成「安全性限制」誤報根修（RECITATION 可重試＋重取樣、升溫+改寫 opt-in 僅 deck 路徑、抽取端 config 逐位元不變）＋MAX_TOKENS 退化迴圈重取樣＋Studio 編輯器（畫布置中零位移/TABLE 表單 0 截字/縮圖獨立捲）。經 /code-review（L20 三鏡頭交叉命中範圍污染→拆兩層）＋/simplify（套 5 跳 1）＋復驗。）前版（2026-07-31 全站重設計套用＋會中待講清單全鏈＋新 slide 模板）：server **`meetcopilot-server-00027-nkz`**、web **`meetcopilot-web-00028-qrl`**（admin 維持 `00001-qf8` 不動）；commits `a022c80`（checklist C1）＋`ea05daf`（C2 匯入抽字）＋`f299ca5`（slides 模板全鏈）＋`23f7071`（server 端點）＋`bdaa4c4`（web 全站重設計）＋`8ce4c3c`（docs）；build server `50eda524-e3ee-4174-b7ec-3ec52602aca5`／web `0284f1b1-109a-4784-b2e6-0be342f3cc15` 皆 SUCCESS；**migration 023 於本次 boot 首次套上 Cloud SQL PG**（`ready:true` 佐證＝boot+migrate 未 crash，PG 方言債關閉）；**shared/crm/server/web 皆重建**；server `services update --image` 保 env（`--no-cpu-throttling` 保留）；web `run deploy` 帶 `NEXT_PUBLIC_API_BASE`；新 env 選配 `ORG_MONTHLY_BUDGET_USD`（未設＝spend 預算條不顯示，屬預設行為）＋`TEXT_EXTRACT_*` 三項（皆有預設）；冒煙全綠：`/api/health` 200＋`/api/ready` `{"ready":true}`＋web `/`307·`/zh-TW`·`/zh-TW/{train,crm,copilot,present/start,spend}` 全 200＋未帶 token `/api/train/personas`·`/api/org/usage` 401；開機 log 無 error（CSE/YOUTUBE 缺鑰警告屬既知良性）。內容＝會中待講清單（migration 023＋三路勾稽＋建會表單）＋C2 匯入抽字＋讀圖 fallback＋全站重設計（淺色雙主題/11 畫面/批准卡兩型/stage 零 chrome）＋slide 模板 6→8（會中 AI 自動選新版式）＋usage budget/by-meeting/train lastScore。）前版（2026-07-25 登入頁移除帳密、純 Google 登入＋admin 後台首次上線）：web **`meetcopilot-web-00027-tvb`**、admin **`meetcopilot-admin-00001-qf8`**（新建第三個 service、首次部署）、server **`meetcopilot-server-00026-xwl`**（僅 `--update-env-vars` 補 `ADMIN_ORIGIN`＋`PLATFORM_ADMIN_EMAILS=martin99097@gmail.com`，image 不動、其餘 20+ env 保留）；commit `3c7e113`；build web `9ca3f161-7a79-484a-a2dc-cd67d195f6ea`／admin `63375e06-2797-4671-8ba5-19e079098595` 皆 SUCCESS；**純前端重建（apps/web AuthForm＋apps/admin login 移除帳密 UI、改純 Google）、無 migration、web/admin 無新相依**；web/admin build 帶 `_GOOGLE_CLIENT_ID`（bake `NEXT_PUBLIC_GOOGLE_CLIENT_ID`）＋deploy 帶 `--set-env-vars=NEXT_PUBLIC_API_BASE`；後端 `/api/auth/login`·`/register` 與 DB `password_hash` 暫留不動（本次僅拔前端 UI）；冒煙 server `/api/health`·`/api/ready` 200、web `/`→307·`/zh-TW`→200·`/zh-TW/login`→200、admin `/`→200·`/login`→200、admin CORS preflight `/api/admin/health`→204＋ACAO=admin 網址、server env 未吹（DB/CORS/Google/Gemini/OpenAI/WS 皆在）。⚠️ **待使用者一次性設定**：Google OAuth Console 把 admin 網址 `https://meetcopilot-admin-54139295474.asia-east1.run.app` 加進該 client「已授權 JavaScript 來源」（不可結尾 `/`、約 5 分鐘生效），否則 admin Google 登入 `origin_mismatch`。**尚未 push origin**。）前版（2026-07-25 對練語速拉桿移到頂部狀態列〔右上角、不用捲即可見〕）：web **`meetcopilot-web-00026-94c`**（server 維持 **`meetcopilot-server-00025-4x9`** 不動）；commit `a881262`；build web `f1e6be4d-58f5-43b5-b024-98e4912e8ae3` SUCCESS；**純 web 重建、無 migration/env/相依**；`run deploy` 帶 NEXT_PUBLIC_API_BASE；使用者回報「找不到拉桿」→ 拉桿由 `mc-call__foot`（底部、頁長要捲到底）移到 `mc-call__bar` 頂列（難度後、marginLeft:auto 靠右），isLive 一連上即顯示；tsc 0；冒煙 web `/zh-TW/train` 200。**尚未 push origin**。）前版（2026-07-25 對練加語速拉桿：前端播放倍速 0.5–2.0× 無段即時可拖）：web **`meetcopilot-web-00025-7r6`**（server 維持 **`meetcopilot-server-00025-4x9`** 不動）；commit `c763775`；build web `314280ee-037d-4e5d-977a-19b9c94a1acf` SUCCESS；**純 web 重建、無 migration/env/相依**；`run deploy` 帶 NEXT_PUBLIC_API_BASE；liveClient `setPlaybackRate`（clamp[0.5,2]＋即時套 activeSources）＋`playPcm` 套 rate＋`nextPlayTime=startAt+buf.duration/rate`（無縫排程隨倍速）；TrainCall 對練中拉桿（即時可拖、換場歸 1.0×）；native Live 無 rate 參數故走前端播放倍速（AskUserQuestion 選定，接受變聲/直播微瑕；先前 prompt 三段已退）；驗證 tsc 0＋next build 0；冒煙 web `/zh-TW/train` 200。**尚未 push origin（deploy 紀錄）**。）前版（2026-07-25 修 /train 對練 AI 無聲〔playbackCtx resume〕＋使用者逐字簡→繁〔opencc〕）：web **`meetcopilot-web-00024-stp`**（server 維持 **`meetcopilot-server-00025-4x9`** 不動）；commit `e8098d1`；build web `b8d9f8a9-1f1f-407b-a5f4-1cf2bcd33f3f` SUCCESS；**純 web 重建、無 migration/env、+opencc-js 相依**；`run deploy` 帶 NEXT_PUBLIC_API_BASE；Bug2＝playbackCtx autoplay 下 suspended 未喚醒（initAudio+playPcm resume）、Bug1＝Gemini input ASR 簡體（opencc-js 動態 import 只轉 rep 簡→繁）；/simplify 套 1＋/code-review 2 視角 raw5→0 高信心；tsc 0＋next build 0；冒煙 web `/zh-TW/train` 200。**尚未 push origin（deploy 紀錄）**。）前版（2026-07-25 修 /train 麥克風無法啟動：CSP script-src 加 blob:〔AudioWorklet 被擋〕＋GSI style-src）：web **`meetcopilot-web-00023-9hb`**（server 維持 **`meetcopilot-server-00025-4x9`** 不動）；commit `1166f24`；build web `6d27e18e-51ef-4d7c-b5d1-b9c082e746e3` SUCCESS；**純 web 重建、無 migration/env/相依**；`run deploy` 帶 NEXT_PUBLIC_API_BASE；根因＝對練 PCM 擷取 AudioWorklet 從 blob: 載入、被 CSP `script-src`（缺 blob:）擋 → `getUserMedia` 前即 `AbortError` → 麥克風權限視窗不跳（實機 console MIC-TEST 確認卡在 addModule）；修＝`script-src` +`blob:`＋`style-src` +accounts.google.com（GSI 樣式）＋`liveClient` 錯誤細分/console.error；驗證 `curl -sI` 確認 `script-src ... blob:`＋`style-src ... accounts.google.com`。**尚未 push origin**。）前版（2026-07-25 對練情境模式〔銷售/合作/政府/面試〕＋可變維度評分＋語言設定〔中文/英文/自動〕＋報告跟 i18n）：server **`meetcopilot-server-00025-4x9`**、web **`meetcopilot-web-00022-ztw`**（commit `cb33d14`；build server `3f5d2755-e8fd-412b-ac80-cadda44a03ca`／web `d599f06a-1574-4f84-95be-bb54167ffb02` 皆 SUCCESS；**新 migration `022_train_mode` 開機自動套**（`training_sessions.mode TEXT NOT NULL DEFAULT 'sales'`；`ready:true` 佐證 boot+migrate 未 crash）；**shared/crm/server/web 皆重建**；`TRAIN_MODES` 資料驅動登錄表〔framing/stance/coachRole/dimensions 單一真相、可擴充〕＋評分改 labeled 陣列〔舊報告向後相容〕＋`TrainLang`〔AI 回覆語言 zh/en/auto 預設中文、專有名詞保留原文〕＋評分報告語言跟 next-intl locale；mode/lang server 權威鎖 token；server `services update --image` 保 env；web `run deploy` 帶 NEXT_PUBLIC_API_BASE；驗證 Workflow 五視角對抗式審查 8→1 已修＋typecheck 三端 0＋server vitest 53 檔 306 測＋crm 9 檔 70 測全綠；冒煙 `/api/health`+`/api/ready` 200＋web `/zh-TW`·`/zh-TW/train`·`/zh-TW/crm` 200＋未帶 token `/api/train/personas` 401。**尚未 push origin**。）前版（2026-07-24 語音對練 Gemini Live 微調：麥克風 chunk 100ms→32ms 降延遲＋每 persona 依 contactId 穩定選 prebuilt 嗓音〔speechConfig 鎖進 ephemeral token〕）：server **`meetcopilot-server-00024-ngk`**、web **`meetcopilot-web-00021-5kg`**（commit `d73b1c4`；build server `b3d8add8-3d78-4e3f-a812-296da3d0c9ba`／web `c6996ff1-efb2-42a2-a36a-f7b1e8d9ccc5` 皆 SUCCESS；**無 migration/env/相依**；即時對話/對練 model 確認＝`gemini-3.1-flash-live-preview`〔config 預設＋deployed env 皆是〕；對照官方 `google-gemini/gemini-live-api-examples` 稽核 Live client 已相容〔多 part 事件正確遍歷、interrupted 清佇列、自動 VAD、ephemeral token、resume/goAway 續連〕，本輪只補「官方最佳實務、我方偏離」兩點＝chunk 延遲＋persona 嗓音；server `services update --image` 保 env（`GEMINI_LIVE_MODEL` 部署後仍在）；web `run deploy` 帶 NEXT_PUBLIC_API_BASE；驗證 apps/web+server tsc=0＋server vitest **51 檔 289 測**全綠〔新 `persona-voice.test.ts` 4 測〕；冒煙 `/api/health`+`/api/ready` 200＋web `/zh-TW`·`/zh-TW/train`·`/zh-TW/crm` 200。**尚未 push origin**〔2 commit `2855c46`+`d73b1c4` 待推〕。）前版（2026-07-24 train 頁自助建對象：#1 AI 補齊真人 persona＋#4 AI 虛擬人物＋每次對練情境目的＋中文名/深連結小修）：server **`meetcopilot-server-00023-hgs`**、web **`meetcopilot-web-00020-mz9`**（commit `2855c46`；build server `3b183fc6-61cd-44ea-abe3-4015eb082cfc`／web `d077e3a0-ad67-4f10-9d70-ed6ec9fce209` 皆 SUCCESS；**新 migration `021_synthetic_contact` 開機自動套**（`contacts.is_synthetic INTEGER NOT NULL DEFAULT 0`；`ready:true` 佐證 boot+migrate 未 crash）；**shared/crm/server/web 皆重建**；server `services update --image` 保 env（未吹）；web `run deploy` 帶 NEXT_PUBLIC_API_BASE；改動＝train 頁自助建對象（persona-gen LLM helper＋`/personas/:id/draft`〔applyAiDraft 未驗證草稿守 §11〕＋`/synthetic`＋buildPersonaPrompt unlocked/objective＋兩端點補 rate limit）＋web（模式切換/AI補齊/虛擬建立器/objective/CRM 虛擬 badge）；驗證 Workflow 六視角對抗式審查 11→2 高信心全修（4 誤報濾掉）＋server vitest 50 檔 285 測＋crm 8 檔 67 測全綠＋typecheck 三端 0；冒煙 `/api/health`+`/api/ready` 200＋web `/zh-TW`·`/zh-TW/train`·`/zh-TW/crm` 200＋未帶 token `/api/train/personas` 401。**尚未 push origin**。）前版（2026-07-24 WYSIWYG Studio 編輯器 C1：文字類就地編輯＋區塊工具列＋右側友善標籤）：web **`meetcopilot-web-00019-5cb`**（server 維持 **`meetcopilot-server-00022-6kr`** 不動）；commit `d794ef4`；build web `67a2d7b8-7cb2-4141-967e-f20e9451e3cf` SUCCESS；**純 web 重建（server/shared/crm 零改動）**；**無 migration/env/相依**；`run deploy` 帶 NEXT_PUBLIC_API_BASE；SlideRenderer 抽 export `renderSlideBlock`/`slideClass`（純改名/抽出→present/thumb/sim 輸出逐字等價）＋新 `EditableSlide`/`ui/InlineText`/`slide-block-ops`；驗證 web typecheck＋build 18 路由、/simplify 4 項＋/code-review 5 視角無 ≥80（含 present/thumb/sim 回歸逐字確認）；冒煙 `/api/ready` 200＋web `/zh-TW`·`/zh-TW/studio`·`/zh-TW/present`·`/zh-TW/sim` 200。**C2/C3 待續**（stat/bullets/features 就地化＋視覺 IconPicker → chart/two-col 收尾＋淘汰 BlockEditor）。詳見 `docs/CRM_UPGRADE_PLAN.md` 外的 WORKLOG WYSIWYG 條目。）前版（2026-07-23 features 3 要點三角形排版：2 上＋1 置中下，螢幕＝匯出一致）：server **`meetcopilot-server-00022-6kr`**、web **`meetcopilot-web-00018-g98`**（commit `0e7158e`；build server `2693ff4f-e10b-4976-9724-bd3e47809144`／web `5c9df610-8460-4e9d-9a80-787e53091f1a` 皆 SUCCESS；**無 migration/env/相依**；`SlideRenderer` 加 `feat-count-N` class＋`studio-present.css .feat-count-3` flex-wrap 置中＋pptx `addFeaturesGrid` 落單卡置中（螢幕＝匯出一致）；驗證 web build＋server 48 測＋前後端 typecheck 全綠；冒煙 `/api/health`+`/api/ready` 200＋web `/zh-TW`·`/zh-TW/studio` 200、env count 18 未吹。）前版（2026-07-23 CRM 升級 Phase A Cycle 1：模擬訓練手動解鎖對練＋新增主管補欄＋子表重爬不覆寫人工值）：server **`meetcopilot-server-00021-rxb`**、web **`meetcopilot-web-00017-xxc`**（commit `628b810`；build server `59e2a322-0edc-4e1e-9799-8dbeead8a45e`／web `d90a73e8-b2ba-45ff-839e-bb9b808c2bf6` 皆 SUCCESS；**新 migration `020_training_unlock` 開機自動套**（`contacts.training_unlocked INTEGER NOT NULL DEFAULT 0`；`ready:true` 佐證 boot+migrate 未 crash）；**shared/crm/server/web 皆重建**；server `services update --image` 保 env（`WS_PUBLIC_BASE`＋`--no-cpu-throttling`，count 18 未吹）；web `run deploy` 帶 NEXT_PUBLIC_API_BASE；驗證 crm 67＋server 270 測、web build 18 路由、typecheck 全綠、/simplify 4 項＋/code-review 6 視角無 ≥80；冒煙 `/api/health`+`/api/ready` 200＋web `/zh-TW`·`/zh-TW/train`·`/zh-TW/crm` 200。詳見 `docs/CRM_UPGRADE_PLAN.md`。）前版（2026-07-23 /sim 實測二輪：補充頁繼承匯入配色接 anchor＋版型多樣＋版面預算不裁切）：server **`meetcopilot-server-00020-q28`**、web **`meetcopilot-web-00016-m7l`**（commit `7d77115`；build server `79136327-5f74-4fad-a210-1b73acc97ef7`／web `4f92d4ea-6965-420a-879c-0d08539f56aa` 皆 SUCCESS；**本輪無新 migration、無新 env、無新相依**；改動＝apps/server（`orchestrator` 新 `deckTailSlide` 撈 deck 尾頁當 anchor → 補充頁繼承匯入 theme；`slide-gen` 補充頁 prompt 加版型決策＋版面預算）＋apps/web（`studio-present.css` `.feature__desc` 2 行截斷安全網）；server `services update --image` 保 env（`WS_PUBLIC_BASE`＋`--no-cpu-throttling`，env count 18 未吹）；web `run deploy` 帶 NEXT_PUBLIC_API_BASE；驗證 server 47 檔 269 測＋web build 18 路由＋前後端 typecheck 全綠；冒煙全綠：`/api/health`+`/api/ready` 200＋web `/zh-TW`·`/zh-TW/sim` 200＋`/api/org/usage` unauth 401。）前版（2026-07-20 /sim 實測修復輪：HUD markdown/欄寬、補充頁重疊、匯入抽色風格對齊、pptx WYSIWYG）：server **`meetcopilot-server-00019-mzw`**、web **`meetcopilot-web-00015-kzp`**（commit `606f7ee`；build server `df93c5e7-263f-43a0-9a67-a9edb376aac4`／web `615c9d03-2ae8-40ef-ab27-6d9495cfa964` 皆 SUCCESS；**本輪無新 migration**；**+pngjs 相依**（server 匯入頁抽色，純 JS 無原生建置）；server 走 `services update --image` **保 env**（含 rev 00018 補的 `WS_PUBLIC_BASE`＋`--no-cpu-throttling`，env count 18 未吹）；web `run deploy` 帶 `--set-env-vars=NEXT_PUBLIC_API_BASE`；改動＝apps/web（HUD markdown 共用元件/欄寬單欄、SlideRenderer 抽色 accent-2/-3、studio-present features/two-col 重疊修）＋apps/server（import/palette.ts 抽色、conversion-job 帶 theme、slide-gen prompt、pptx-render chartPalette 對齊螢幕）；驗證 server 47 檔 269 測＋web build 18 路由＋前後端 typecheck 全綠；冒煙全綠：`/api/health`+`/api/ready` 200＋web `/`307＋`/zh-TW`·`/zh-TW/sim`·`/zh-TW/spend` 200＋`/api/org/usage` unauth 401。**中間 rev 00018-47r**＝WS_PUBLIC_BASE 補設（realtime WS 修復，見排錯速記）。）前版（2026-07-20 四輪：DynamicSlide 補充頁橋接+mp3 會議模擬器／AI 記帳完整化並對齊 ezpage／org 花費頁）：server **`meetcopilot-server-00017-dl5`**、web **`meetcopilot-web-00014-qpq`**（commits `569f9ea` feat(server)＋`139dc61` feat(web)＋`b26c682` docs；push origin main `2f98231`→`b26c682`；build server `9a008cb0-7271-48e1-9ea3-4c27709ce641`／web `75d51e1b-42db-4eb0-bd02-217cd23a9213` 皆 SUCCESS；**新 migration `019_usage_detail` 開機自動套**（usage_events 加 reasoning_tokens/cached_input_tokens/retry_count/cost_tax_multiplier；`ready:true`＋`listening on :8080` 佐證 PG migrate() 過——019 若失敗會 crash boot）；**packages(shared/crm)＋server＋web 皆重建**（本 session 動到三者）；`services update --image` 保 env＋`--no-cpu-throttling`；web `run deploy` 帶 `--set-env-vars=NEXT_PUBLIC_API_BASE`；**無新必填 boot env**（`COST_TAX_MULTIPLIER`/`SUPPLEMENT_AUTO_LIMIT_PER_MEETING`/`SUPPLEMENT_THROTTLE_MS` 皆有預設）；冒煙全綠：`/api/health`+`/api/ready` 200＋web `/` 307＋`/zh-TW` 200＋`/zh-TW/spend` 200（新花費頁）＋`/api/org/usage` unauth 401（新端點 mounted+guard）＋`/api/auth/google` bogus 401；開機 log 無 error（GOOGLE_CSE/YOUTUBE/PLATFORM_ADMIN 缺鑰 warning 屬預期）。待使用者：本機用 `/sim` 模擬器需 GEMINI_API_KEY＋poppler；`/spend` 需 owner/admin 登入且跑過 AI 才有數字。）前版（2026-07-19 三指令輪：筆記 md 渲染／社群真內容／照片 v3）：server **`meetcopilot-server-00016-dtp`**、web **`meetcopilot-web-00013-w8v`**（commits `ab7f3ed` feat(research) 社群真內容——YT 無金鑰抓取/Threads 推導＋登入牆偵測/FB·IG AI 摘要/二次抓取＋筆記來源全解析＋照片 v2/v3（DOM 鄰近＋Google CSE）、`822923f` feat(web) 筆記 markdown 渲染（react-markdown/無 raw HTML/連結白名單）、`0dcef09` docs；push origin main `4d8d78d`→`0dcef09`；build server `f15ce324-99f5-4895-91b7-fbb9d9a72fbb`／web `0f1c0d68-6b8b-463a-8398-796a41f1d66a` 皆 SUCCESS；**本輪無新 migration**（016/017/018 前次部署已套、開機 log 無 migration 動作）；`services update --image` 保 env＋`--no-cpu-throttling`；冒煙 `/api/health`+`/api/ready` 200＋web `/` 307＋`/zh-TW` 200＋開機 log 無 error（GOOGLE_CSE/YOUTUBE 缺鑰 warning 屬預期））。前版（2026-07-19 DynamicSlide 匯入重構＋研究/社群/會中導覽擴編）：server **`meetcopilot-server-00015-xbb`**、web **`meetcopilot-web-00012-xmx`**（merge `55b812c`＝並行 research/社群/導覽工作 `ee2468a`＋DynamicSlide 匯入重構 `bf88fd9`，push origin main；build server `b98dad17-c0b6-4652-beb2-44527a68ca7d`／web `8d5e7f6a-5686-4dba-95cd-2a36d3e67c45` 皆 SUCCESS；**新 migration 016_social_tech/017_more_mode/018_deck_import 開機自動套**（ready:true＋reaper 查 import_jobs/deck_assets 無錯佐證）；**server Dockerfile 新增 LibreOffice-impress+poppler-utils+fonts-noto-cjk（image +~1GB，供 pptx/pdf→PNG 轉檔）**；DynamicSlide 新選配 env 皆有預設未動；`services update --image` 保 env＋`--no-cpu-throttling`；冒煙 `/api/health`+`/api/ready` 200＋web `/` 307＋`/zh-TW` 200＋開機 log 無 error＋3 reaper 0 孤兒）。前版（2026-07-18 研究引擎擴編）：server `meetcopilot-server-00013-8ms`、web `meetcopilot-web-00011-89t`（commits `6b6025f` feat(research) 引擎擴編——雙語查詢/五新角度/升模/深讀12/渲染 fallback/per-contact/商機/產品外部回填/二段式深抽/MAX_TOKENS/504 修復＋`c38291d` feat(web) ProductsTab 明細補 render＋`0d9c95c` docs；build server `fbec48f0`／web `ac85d0d8` 皆 SUCCESS；本輪無 migration；冒煙 `/api/health` 200＋`/api/ready` 200＋web `/` 307＋`/zh-TW` 200、開機 log 無 error、reaper 0 筆孤兒；`--no-cpu-throttling` 保留）；前版 server 00012-drd／web 00010-lmf（`6826567` feat(research)＋`92020ed` feat(web)＋`95de538` docs：CRM 品質四修——migration 015 五欄＋爬蟲抓圖＋型號/中文名/繁中優先 UI＋研究 job 開機 reaper；`--no-cpu-throttling`＝CPU always-allocated 已套、保留至今）。
