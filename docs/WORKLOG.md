...
- **狀態**：**未 commit／未部署**（硬規則 10，待使用者核准）。變更集＝train 自助建對象 23 檔＋docs 2（CHANGE_TRACKER 2 筆〔小修＋大功能〕、ROM 1 筆、CRM_UPGRADE_PLAN Phase A2 節）。**含 migration 021**（開機自動套）→ 部署時 server＋web 皆須重建。
- **待辦**：使用者核准後 commit＋部署（server `services update --image` 保 env、web `run deploy` 帶 NEXT_PUBLIC_API_BASE）；ROM 已 671 行（逾 500）**待歸檔**至 `rom_archives/ROM_001.md`（下個維護窗口做）。
