...
## 2026-08-19 session（會中收音大改：雙聲道 speaker 分軌＋結束會議＋殭屍會議防護封閉）

- **起點（使用者看 /copilot 截圖問）**：「目前 Google Meet 或 LINE 會議的語音對話，有辦法錄到音嗎」＋隨後補「並且少了中止會議的選項」。兩個問題各自挖出一條線。
- **錄音可行性的地面真相**：現行 pipeline 只有 `getDisplayMedia`（`audio-capture.ts`），抓的是 **Meet 分頁播放出來的混音＝遠端與會者**；Meet 為避免回授不會把本機麥克風回放進分頁，故**報告者自己講的話完全不在音軌裡**。LINE 桌面版通話是原生程式、沒有分頁可分享，LINE 也無官方通話媒體 API → 純瀏覽器唯一可能是「整個螢幕＋系統音訊」，而 W3C spec 明講 user agent 可以不給音訊、**無跨平台保證**（`API_FINDINGS.md` 至今無此條目）。
- **使用者拍板（四選項中選最貴的一條）**：麥克風合流走 **stereo 雙聲道**（L＝麥克風＝報告者、R＝分頁＝對方），後端 deinterleave 雙路 ASR，換取逐字稿天然帶 speaker 標記。知情接受 ASR 成本約翻倍。
- **兩個讓改動面大幅縮小的設計判斷（ROM 12:05）**：(a) **deinterleave 放 `hub.pushAudio` 最上游**，chunker 以下整條鏈維持純 mono → 破壞點清單第 1–14 項整批消失（否則 `chunker.ts` 的音訊時鐘會跑兩倍快，而 `checklist.test.ts` 的時鐘測試繞過 PCM 路徑、**抓不到那個迴歸**）；(b) `TranscriptSpeaker` 既有 enum `presenter|client|unknown` 與左右聲道**天然對應** → 零 enum 新增、零 DB migration、HUD 的 CSS class 與 i18n key 全部沿用。
- **實作 agent 比契約更好的兩處**：時鐘所有權**上移到 session 層**而非「建右軌時同步左軌 consumedSamples」（同步法在 mono→stereo→mono→stereo 的第二次切回會失效）；發現 `hub` 原本讀的是 **final 抵達當下**的 `audioChannels`，而轉寫非同步＋chunker 累積 4 秒 → **對飛行中的音訊永遠是錯的**，改成段落在切段起點快照模式帶著走。
- **「結束這場會議」＝後端早就有、前端從未接**（`api.ts` 的 `endMeeting` 全 `apps/web` 零呼叫者）。接線時挖出殭屍會議坑：server 用 close code **1000** 關 socket，而前端只把 4000/4001 判 terminal → 自動重連 → 握手不查 status → `ensureRuntime` 替 completed 會議**重建 ASR runtime＋consent 重置為 false**。
- **/code-review（18 agents，五視角→逐項對抗駁倒）**：**confirmed 0，但 L20 第四度生效**——killed 裡 9 項 `refuted:false`，逐條裁決後**修 7 項**，其中一項是 CRITICAL：**1000 判 terminal 只改了 `useRealtime`，`/present` 自帶一份 close-code 表沒跟上**（整套防護做了一半）。另 `hub.ts` 兩處各被兩個互不知情的視角命中（右軌時鐘從 0 起算 → 分析窗**單向清空客戶那一路**；模式切換時左軌殘料被貼 `presenter` 並**落庫**）。
- **殭屍會議的真正封閉（前端做完才發現的根因）**：`readMeetingCreds()` 先讀 URL query，而 `/hud`、`/present` 憑證就在網址列 → **按一次 F5 就是全新連線，前端所有閘全繞過**。修＝WS 握手加 **org-scoped meeting-status 閘**（`ws-handshake-gate.ts`），併進帳號閘既有的那一次 `db.get`（**零額外 DB 往返**），跨 org 與「不存在」回應**逐位元相同**（機械驗證）。測試直接斷言 `hub.getRuntime()` 保持 undefined＝驗根因非表象。
- **/simplify（四角度並行→做 13 跳 4 記債 1，分四批）**：靜音軌不再空打 Gemini（雙軌後**正常輪流講話就代表幾乎整場有一條軌靜音**，約 3,600 次/小時零產出呼叫）／分析窗改字元預算（原本 10→20 讓 **mono 也替 stereo 付錢**，而 mono 正是麥克風被拒的 fallback）／close-code 與 `channels` 收進 `packages/shared`（批 A 另找出**第三份 fail-safe 複本**，四個審查角度都沒抓到）／`consumedSamples` 死碼＋**五處失效註解**（兩處是本輪剛寫的新檔）／測試 helper 收進 `test-support.ts`。
- **本輪抓到的既有問題（非本輪引入）**：`npm test` 原約 **50% 機率**間歇失敗，根因＝Windows 上 vitest 因磁碟機代號大小寫把同一模組求值兩次 → `instanceof` 判偽、prototype spy 靜默不觸發（**入 LESSONS L23**，並確立「**連跑三次才算綠**」的驗收慣例）；`ws-async-gate.test.ts` 的 `slowCore` 讓握手一直被擋、`hub.attach` **從未被呼叫**＝整檔空洞通過。
- **驗證方法上值得留的三件事**：批 C 對 worklet 做**逐位元組差分**（240 組態／2708 frames）＋**四種 L/R 錯位突變全被抓到**；批 B 用三個突變反證新測試不空洞；批 D 的 `passingHandshakeRow()` 把「握手閘多讀一欄 → 測試靜默 vacuous 卻全綠」換成**編譯錯誤**。
- **記債／待辦**：(1) **peak 門檻要用真錄音校準**——靜音軌優化用 `SILENCE_RMS_THRESHOLD=400`，若真實麥克風底噪 peak 超過它，該優化**退化為 no-op**（失效方向安全，但省不到）；(2) `endingMeeting` 防重入讀的是 stale state、**至今名義上有效**（真修法＝`useRef`，屬行為改變）；(3) **worklet 驗證腳本仍在 scratchpad、`npm test` 跑不到**，而 `apps/web` 完全沒有 test script → 下一個動 `pcm-worklet.js` 的人會在零測試佐證下改全 repo 最不該手滑的地方；(4) 前端終態判定散在 4 處（統一到 `useMeetingSession()` 的重構已評估，因需重排 creds 所有權而**刻意記債**）；(5) LINE／系統音訊**待使用者實機實測**（`tools/capture-test.html` 的「測試 D」已備妥，勾「分享系統音訊」需真人手勢，結果要回寫 `API_FINDINGS.md`）；(6) server 端 `{type:"error"}` 的 message 仍硬編繁中（4003 亦然）。
- **最終數字（獨立 fresh-context agent 跑的最終回歸，零修改）**：typecheck 五 workspace 全過；`npm test` **五連綠**（要求三次，多跑兩次涵蓋 build 前後）＝ crm **11 檔 88 測**、server **70 檔 514 測**，L23 間歇失敗一次都沒重現；`apps/web` `next build` 19 路由零 warning；worklet 三支腳本 **50 項 PASS ＋ 240 組態/2708 frames 逐位元組相同 ＋ 四種 L/R 錯位突變全被抓到**（三支已從 scratchpad 搬進 `tools/`，見下）；i18n zh/en 各 507 key 雙向差集為空；`git diff HEAD --stat` = **40 檔、+2874/−903**（另 6 個未追蹤新檔）。工作樹無意外檔案（無 scratchpad 外洩／`.bak`／`dist` 誤入版控）。
- **驗證 agent 額外抓到的一件事**：第一次 `npm run build -w @meetcopilot/shared` 其實是**假的**——`tsconfig.json` 有 `composite: true`，`dist/protocol.js` 比 `src/protocol.ts` 新，tsc 走 incremental 直接跳過、什麼都沒寫。刪 `dist/`＋`tsbuildinfo` 做 clean rebuild 後 44 檔 sha256 比對 IDENTICAL，才證明那份 dist 真的是新的。**日後驗證 shared 是否已重建，不能只看 build 有沒有 exit 0。**
- **部署注意（新增）**：`packages/shared/dist/` 需重建——server vitest 經 node_modules symlink 吃 `dist` 不吃 `src`，**部署前 shared 要先 build**（且注意上一條的 incremental 陷阱）。
- **worklet 驗證腳本已入庫**（債務 (3) 清償）：三支從 scratchpad 搬進 `tools/`（路徑改成相對 `import.meta.url`，斷言邏輯一字未動），`tools/README.md` 補一節說明「為什麼這支檔特別危險／何時必須跑／怎麼判讀三種結果」，`pcm-worklet.js` 檔頭加一行指路。**未掛進 `npm test`**（`apps/web` 無 test runner，本輪決定不另建 vitest）→ **CI 不會替你跑，動 worklet 前後要自己跑**。
- **基準檔的關鍵發現**：`worklet-diff.mjs` 的比對基準**無法從 git 保全**——`git show HEAD:pcm-worklet.js`（stereo 之前的 mono 版）、scratchpad 基準（stereo 完成、simplify 之前）、working tree 三者 md5 互不相同，基準是**未進過任何 commit 的 working-tree 中間態**。已凍成 `tools/pcm-worklet.baseline.js`。理由不只「git 拿不到」，更重要的是**git 相對基準是會移動的靶**：`git show HEAD:…` 會在破掉的改動一被 commit 的瞬間自動轉綠，**恰好在最需要它的時候停止防守**。⚠️ **本批 commit 時 `tools/pcm-worklet.baseline.js` 必須與 `pcm-worklet.js` 進同一個 commit**，否則 `worklet-diff.mjs` 對任何 clone 都跑不起來。
- **commit＋push＋部署（2026-08-20 使用者核准「1 2 都做」）**：兩 commit `e7d6a05` feat(realtime)＋`8f351bc` docs，push origin main（`5bab897..8f351bc`）。build server `95526688-7b5b-4609-897a-1f69072f942b`／web `786efc9f-e04e-4dea-b2fc-dd99f55d9c5f` 皆 SUCCESS → **server rev `00030-ssn`**（`services update --image` 保 env，部署後 env count 20 未吹）＋**web rev `00031-wxv`**（`run deploy` 帶 NEXT_PUBLIC_API_BASE）。無 migration、無新必填 env。冒煙全綠：server health 200＋ready true＋unauth 401；web `/` 307＋六路由 200＋**`/pcm-worklet.js` 200 且內容驗明為新版**（`joinCarry`×5＋權威方向註解＋overshoot clamp，證明非快取舊檔）；server 開機 log 無 error。DEPLOY.md 版本節已更新。
- **⚠️ 本次新增的部署陷阱（已寫進 DEPLOY.md，日後同類改動照辦）**：**server 必須先上、web 後上**。新 web 在握手帶 `channels=2` 並送交錯 stereo PCM，舊 server 不認得該 param → fail-safe 當 mono → 交錯資料被當單聲道解讀＝**嚴重混疊噪音＋音訊時鐘跑兩倍快，且是靜默失敗**。反向順序安全（新 server＋舊 web＝舊 web 不送 param、落 mono、資料也真的是 mono）。本次即照此順序部署並在 server 上線後先驗 `ready:true` 才動 web。
