...
## 一個共用核心＋三個消費端（產品包裝＝兩個 surface：DynamicSlide、MeetCopilot〔含會中副駕＋模擬訓練〕）
...
- 研究引擎（混合式）：
  ...
  2. **自建爬蟲（搬 ezpagesite wizard「從網址匯入」）**：Playwright + stealth 渲染 SPA、子頁連結評分爬取（最多 5 子頁）、視覺截圖；深讀 rep 指定的對方官網。**雙模式分工：quick（單頁、輕量）＝會中即時研究用；detailed（子頁＋截圖）＝會前 CRM 建檔用**。**補上 ezpagesite 缺的 SSRF 防護**（把檢查掛在對使用者提供 URL 的首次 fetch，沿用 v1 SSRF-safe 抽取器）。原版實作地圖見 [research/EZPAGESITE_CRAWLER.md](research/EZPAGESITE_CRAWLER.md)。
...
| 上送 + ASR final（**MVP＝Gemini 分段轉寫**） | **~2–5 s**（受分段長度支配；升級串流 STT 後可到 <1 s，AsrProvider 換 impl 即可） |
| 增量分析（LLM） | ~0.8–1.5 s |
| CRM 檢索 | ~50–150 ms |
| 副駕建議浮現（**自 final segment 起算**） | **< 2 s** |
