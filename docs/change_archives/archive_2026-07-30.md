# CHANGE_TRACKER 歸檔 — 2026-07-30

> 涵蓋 2026-07-19 ～ 2026-07-30（41+2 筆）。主題：會中進行收斂（cockpit）＋補充頁 theme 繼承；CRM×DynamicSlide×模擬訓練多階段升級（Phase A1/A2/A3：手動解鎖對練、train 自助建對象、情境模式＋對練語言）；WYSIWYG Studio C1；Gemini Live 3.1 微調＋語速拉桿；登入純 Google（web+admin）＋admin 首次上線；**會中待講清單全鏈（migration 023＋三包＋三輪 code-review 修正：slideIdx 座標系、evidence TTL 縱深、建會限流、uncheck 音訊時鐘冷卻、記帳歸屬）**；「會中進行」兩入口改造（會議簡報/MeetCopilot 改名、/present/start 準備頁、舞台全螢幕）；/simplify 十項行為不變清理。
> 2026-07-30（歸檔後）起之新紀錄留於 `docs/CHANGE_TRACKER.md`。

---


### 2026-07-30 15:20 | /simplify 四鏡頭清理套用（10 項行為不變重構；3 項判定跳過）
- **工作區**: apps/server, apps/web, packages/crm, packages/shared
- **類型**: refactor
- **檔案**: `apps/server/src/gemini.ts`, `apps/server/src/generation/checklist-gen.ts`, `apps/server/src/research/deep-extractor.ts`, `apps/server/src/realtime/hub.ts`, `apps/server/src/realtime/ws-server.ts`, `apps/server/src/realtime/ws-presenter-authz.test.ts`, `packages/crm/src/repos-checklist.ts`, `packages/crm/src/repos-decks.ts`, `packages/shared/src/checklist.ts`, `packages/shared/src/deck.ts`, `apps/web/components/hud/ChecklistPanel.tsx`, `apps/web/components/present/PresentStage.tsx`, `apps/web/app/globals.css`
- **改了什麼**（全部行為不變；測試斷言零改動）:
  - **A. `isMaxTokensError` 三份複製收斂**：gemini.ts 新增 `export function isMaxTokensError`（錯誤訊息 `finishReason=MAX_TOKENS` 的產生者擁有判定式）；checklist-gen.ts:126 與 deep-extractor.ts:736 的逐字相同本地版刪除、改 import（兩檔原本就 import gemini.js，type-only → 加值匯入）。decks-routes 的 `/MAX_TOKENS/i.test(msg)`（輸入是 string 非 Error）刻意不動。
  - **B. hub.ts metered Gemini 組裝單一建構點**（reuse／simplification／efficiency 三鏡頭都點名）：新增 private `checklistDeps({orgId,userId?,meetingId?,idemPrefix})`；`checklistGenDeps`（公開簽名不動）與 `runChecklistGeneration` 皆委派。kind='gemini_text'／fallback 規則從此只有一份；「為何 idemPrefix 不同／必帶 userId」的記帳註解留在各呼叫點。`meetingId: undefined` 與省略等價（MeteredGeminiCtx 只讀屬性值）。
  - **C. hub.ts checklist wire 訊息單一建構點**：新增 private `checklistMessage()`；`broadcastChecklist`（I3 廣播出口）與 `sendChecklistSnapshot`（單 socket 補發）共用，契約 §5 形狀不再有兩個字面值。snapshot 的 `if (!phase && items.length===0) return`／`readyState` 守衛／單 socket 送出（不走 broadcast）全部原樣。
  - **D. 「must 優先、再依 idx」comparator 收斂到 shared**：`packages/shared/src/checklist.ts` 新增 `export function compareChecklistOrder`（＝hub 原 comparator 逐字搬移）；hub.ts `refreshPendingChecklist` 的 `.sort(...)` 與 ChecklistPanel.tsx `nextPending`（原單趟掃描，strict `<` ＝ stable sort 取第一筆，等價）改共用——HUD「下一個待辦」與模型 pending 注入順序從此同源（§7.1＝§8）。
  - **E. repos-checklist.ts `setStatus` 死三元清除**：`coveredBy`/`coveredAt` 的 `status==="covered" ? ... : null` 中 `: null` 臂永遠讀不到（非 covered 分支 SQL 直接寫死 NULL）→ 改 `const coveredBy = by ?? "manual"`、covered 分支參數用 `now`；SQL 文字與參數順序完全不動。
  - **F. PresentStage.tsx 全螢幕 vendor-prefix 收斂**：模組層新增 `FsDocument`/`FsRootElement` 型別＋`fsElement()`/`fsActive()` 純 helper；支援偵測 effect 與 `toggleFullscreen` 共用（原兩處各自重寫 as-轉型與 active 判定）。request/exit 呼叫順序與 Promise 吞錯不動。fs/hint/uiOn 三組狀態語意不同，依審查建議不合併。
  - **G. globals.css 死 CSS 四處刪除**：`.mc-sidebar__ext`（:471）、rail 隱藏清單中的 `.mc-shell.is-rail .mc-sidebar__ext,`（:536）、`.mc-surfacecard__ext`（:1481）、`.mc-cockpit__second-qr`（:1522）——本輪 UI 移除「↗」span 與假 QR 方塊後全站已無元素命中（grep 證零使用）。
  - **H. `runChecklistGeneration` 用 `replaceAll` 回傳值**：Before 丟棄回傳值後 `broadcastChecklistSnapshot`（重跑一次全量 SELECT）；After `const stored = await replaceAll(...)` → `this.broadcastChecklist(meetingId, "ready", stored)`（phase 上一行剛設 ready，與 snapshot 的 `?? "ready"` 等價；內容/順序/hud-only 相同），省一次 DB 往返。`refreshPendingChecklist` 的讀取維持原樣。
  - **I. ws-server.ts `onPageCommitted?.` 改直呼**：ws-presenter-authz.test.ts 的 makeFakeHub 補 `onPageCommitted: vi.fn()`（checklist.test.ts 的替身本就有；ws-async-gate 的替身不送 page_commit）→ ws-server.ts:187 刪 `?.` 與兩行道歉註解（§7.2 說明保留）。生產端 RealtimeHub 恆有此方法，行為不變。
  - **J. `textExtract` 收進 DeckSlide 域型別**：`packages/shared/src/deck.ts` DeckSlide 加 `textExtract?: string`；repos-decks.ts SlideRow 加 `text_extract: string | null`（查詢皆 `SELECT *`，migration 023 已建欄）＋ rowToSlide 映射 `?? undefined`；checklist-gen.ts:373 刪 `(s as {textExtract?...})` 硬轉、直讀 `s.textExtract`。C1 無任何寫入路徑（`setSlideTextExtract` 尚未實作）→ 執行期恆 undefined、JSON 序列化照舊省略＝今日行為零變化；C2 實作後餵料路徑即通（契約 §6.4/§C2 本來就寫 `slide.textExtract`）。
  - **跳過 3 項**：①transcript-retention.ts TTL SQL 片段抽 helper——收益小且碰剛通過對抗式復驗的隱私資產，照「跳過比套錯好」原則不動；②`setPendingChecklist` 改必要方法——審查自評 risk=medium 且牽動 hub 守門＋engine 替身，不動；③trust.ts:2 註解改名——審查自評「不做也完全可以」，且 docs 內部架構名仍是「會中副駕」，改了反而與制度檔脫節。
  - **驗收（全鏈回歸）**：packages/shared build EXIT=0；packages/crm `tsc -p tsconfig.build.json` EXIT=0＋vitest **10 檔 80 測全綠**；apps/server `tsc --noEmit` EXIT=0＋vitest **60 檔 355 測全綠**（基準 351，未減少）；apps/web `tsc --noEmit` EXIT=0＋`next build` **19 路由**；i18n parity zh-TW/en **301=301、零差異**。deck-outline／transcript-retention／rate-limit-wiring／checklist 冷卻期等資產斷言一律未動。
- **為什麼**: 三輪 code-review 修完後的 /simplify 收尾——把修 bug 過程留下的複製（isMaxTokensError×3、metered 組裝×2、wire 字面值×2、排序規則×2）、死條件（setStatus 三元）、死 CSS、與型別謊言（textExtract as-cast）在行為凍結下清掉，降低下一輪改動的漂移面。

### 2026-07-30 13:55 | uncheck 冷卻期改用「音訊時鐘」計時（契約 §7.5 v1.2；牆鐘版打地鼠復活）
- **工作區**: apps/server
- **類型**: fix
- **檔案**: `apps/server/src/analysis/analysis-engine.ts`, `apps/server/src/analysis/gemini-analysis.ts`, `apps/server/src/realtime/session-runtime.ts`, `apps/server/src/realtime/hub.ts`, `apps/server/src/realtime/checklist.test.ts`
- **改了什麼**:
  - **A. 分析引擎暴露唯讀音訊時鐘（單一真相）**：`analysis-engine.ts:33-45` 介面新增 optional `latestWindowT?(): number | undefined`（optional 是為了讓既有精簡/測試 engine 替身無需改動仍滿足介面）；`gemini-analysis.ts:141-148` 實作 `latestWindowT() { return this.window[this.window.length - 1]?.t; }`——**刻意與 `trimWindow()` 用完全相同的取法**，讓「冷卻是否走完一個窗」與「窗要不要丟掉這一段」永遠拿同一基準。`WINDOW_MAX_AGE_MS` 註解補上時鐘域警語（`gemini-analysis.ts:21-28`）。
  - **B. `session-runtime.ts` 冷卻改音訊時鐘**：新增 `audioClockMs()`（`:150-157`，`engine?.latestWindowT?.()` 雙層 optional chain＋`Number.isFinite` 過濾，取不到回 `undefined`，**絕不 throw**）；`recentlyUnchecked` 的值型別 `Map<string, number>` → **`Map<string, number | null>`**（`:104`）。
    - `noteUnchecked`：Before `noteUnchecked(itemId, now = Date.now())` 存牆鐘；After `noteUnchecked(itemId, audioT = this.audioClockMs())` 存**音訊時鐘高水位**，取不到則存 `null`（掛帳待 pin）。淘汰迴圈改 `at != null && audioT - at >= UNCHECK_COOLDOWN_MS`，且 `audioT == null` 時本輪不淘汰（無從判齡，下次取到時鐘會補淘汰；Map 鍵是 itemId，天然 ≤ 本場清單長度＝仍有界）。
    - `isUncheckCooling`：Before `now - at >= UNCHECK_COOLDOWN_MS`（牆鐘）；After `audioT - at >= UNCHECK_COOLDOWN_MS`（音訊時鐘），並補三個邊界：①`audioT == null`（窗空／engine 沒實作）→ **回 `true`＝仍在冷卻（fail-safe）**；②紀錄值為 `null`（uncheck 當下沒有時鐘）→ 用「第一次讀到的高水位」pin 成基準並回 `true`，冷卻自音訊恢復流動起算（仍有界，不會永久卡住）；③`audioT < at`（時鐘後退，如未來 chunker 重建）→ 回 `true`，同樣落在安全側。過期仍即時 `delete`（放行＋淘汰），`dispose()` 仍 `clear()`。
  - **C. `hub.ts` 呼叫端**：`coverChecklist` Before `const now = Date.now(); … isUncheckCooling(id, now)`；After `const audioT = runtime.audioClockMs(); … isUncheckCooling(id, audioT)`——整批只讀一次時鐘，同一輪分析的所有 id 用同一基準判齡。`checklistAction` 的 §7.5 註解同步改為「記音訊時鐘高水位」。
  - **測試（`checklist.test.ts`）**：§7.5 那組 harness 新增 `advanceAudio(ms)`（往**真** engine `ingest` 一段逐字段推進音訊時鐘，走產線路徑 `hub.onAsrFinal → engine.ingest`；gemini 未設定故 `maybeAnalyze` 直接 return、不打 LLM）與 `audioClock()`。既有 5 條改用音訊時鐘推進（原「把紀錄倒退 `Date.now() - COOLDOWN - 1`」改成真的 `advanceAudio(UNCHECK_COOLDOWN_MS)`）；**新增 4 條**：①存取器語意（窗空→undefined、餵段後＝窗內最新 t）；②**原始缺口回歸**——uncheck 後 `vi.spyOn(Date,"now")` 把牆鐘推 +120 秒而音訊時鐘凍結在 4000ms → 斷言自動 cover **仍被跳過**，再 `advanceAudio(90s+1s)` → 斷言**放行**；③音訊還沒開始流時 uncheck → fail-safe 仍算冷卻、基準改由恢復後第一個高水位起算、走完一個窗後放行；④`dispose()` 清空冷卻紀錄。檔案 21 → 25 測。
  - **驗收**：`apps/server` `npx tsc --noEmit` EXIT=0；`npx vitest run` **60 檔 355 測全綠**（基準 351）。**突變驗證**：把 `noteUnchecked` 預設值與 `coverChecklist` 的時鐘退回 `Date.now()`（其餘不動）→ 3 測轉紅，其中回歸測正是行為面斷言失敗（`expected 'covered' to be 'pending'`＝打地鼠復活重現）；另一版突變（`audioClockMs()` 直接 `return Date.now()`）→ 4 測紅。已全部還原並重跑綠。
- **為什麼**: 契約 §7.5 v1.1 只寫「冷卻 90 秒」而未指定時鐘域（**契約自身的缺陷，非實作的**），實作照字面用了 `Date.now()`；但分析滾動窗的年齡是用**音訊取樣時鐘**算的（`chunker.ts` 的 `consumedSamples / (SAMPLE_RATE/1000)`，只在 PCM frame 進來時前進），兩個時鐘只在音訊持續流動時等價。實測失敗情境：報告者 uncheck 一個誤判項 → 按「撤回同意」做 2 分鐘內部討論（`pushAudio` 在 `!consent` 時 return，音訊時鐘完全凍結，且 consent handler 不清 engine 的窗）→ 牆鐘冷卻已到期，但那段害它被誤判的逐字稿在音訊時鐘上只老了幾秒、**仍在窗裡** → 恢復後第一輪分析（節流 5 秒）就把同一項再劃掉，§7.5 要消滅的打地鼠原樣復活；停止分享導致 capture socket 斷線（HUD 仍在故 runtime 不被回收）亦同。fail-safe 方向選「取不到時鐘＝仍在冷卻」而非「放行」，與 §7.1「寧漏勿誤、誤劃比漏劃傷害大」同向：多擋一下自動勾稽只是晚點自動劃掉（報告者隨時可手動 check），推翻報告者的 uncheck 卻會讓他漏講。

### 2026-07-30 13:20 | checklist 最後一輪修正：建會端點限流缺口＋uncheck 打地鼠＋生成漏帶 userId＋重勾重置 TTL
- **工作區**: apps/server, packages/crm
- **類型**: fix
- **檔案**: `apps/server/src/index.ts`, `apps/server/src/realtime/meetings-routes.ts`, `apps/server/src/realtime/hub.ts`, `apps/server/src/realtime/session-runtime.ts`, `apps/server/src/analysis/gemini-analysis.ts`, `packages/crm/src/repos-checklist.ts`, `apps/server/src/ops/rate-limit-wiring.test.ts`（新）, `apps/server/src/realtime/checklist-metering.test.ts`（新）, `apps/server/src/realtime/checklist.test.ts`, `packages/crm/test/checklist-repo.test.ts`
- **改了什麼**:
  - **A. 限流（index.ts:148-158 ＋ meetings-routes.ts:34-38）**：把 `POST /api/meetings` 與 `POST /api/meetings/draft-objective` 兩條加進 index.ts 既有的共用 exact-path 名單（`app.post(path, jwtGuard, limit)`，**掛在 body parser 之前**）；**移除** meetings-routes.ts 內 router-local 的 `const objectiveLimit = rateLimit(new TokenBucketRateLimiter().start())` 與 `token-bucket.js` import，`router.post("/draft-objective", ...)` 不再自帶 limiter。Before：`/api/meetings` 完全無限流（而它會背景觸發 12k 字 outline／4096 out-tokens／45s deadline 的 `generateChecklist`），且 draft-objective 用**第二個**桶 → 同 org 額度兩桶相加、被 429 的請求仍已 parse 2mb body。After：兩條共用同一個 `rateLimiter` 實例（全 src 只剩 1 個 `new TokenBucketRateLimiter(`），超限在 parse 前就 429。
  - **B. uncheck 冷卻期（契約 §7.5 新增）**：`gemini-analysis.ts:21` 把 `WINDOW_MAX_AGE_MS` 改為 **export**（單一真相）；`session-runtime.ts` 新增 `export const UNCHECK_COOLDOWN_MS = WINDOW_MAX_AGE_MS`、private `recentlyUnchecked: Map<itemId, epoch-ms>` ＋ `noteUnchecked()`／`isUncheckCooling()`（過期即淘汰，Map 有界），`dispose()` 清空；`hub.ts` `coverChecklist` 在 `by === 'transcript'` 時先濾掉冷卻期內的 id（全被濾光即 return，不廣播），`checklistAction` 的 `uncheck` 在 `setStatus` 回非 null 且 `runtime.orgId === orgId` 後 `noteUnchecked(itemId)`。`'slide'` 與手動 `check` 不受限；項目仍留在分析 prompt 的 pending 清單。
  - **C. 生成記帳歸屬（hub.ts:470-479）**：`runChecklistGeneration` 的 `meteredGeminiClient` 補 `userId: binding.presenterUserId`。Before：`usage_events.user_id` 落 NULL（同一次建會的兩個 LLM 呼叫，貴的那個掛「未知使用者」）。After：與 draft-objective 一致歸屬到建會者。
  - **D. TTL 時鐘（repos-checklist.ts:setStatus）**：`covered` 分支的 `covered_at = ?` 改為 `covered_at = CASE WHEN covered_by = ? THEN covered_at ELSE ? END`（與既有 evidence CASE 同風格，SQLite/PG 皆合法，不用 `IS DISTINCT FROM`）。Before：同來源重勾把 `covered_at` 刷成 now → purge 的 `COALESCE(covered_at, created_at)` 年齡歸零，過期列被再點一次就躲過那輪清理。After：`covered_by` 未變化＝什麼都沒變 → 時間戳不動；換來源才更新 `covered_at` 並清 evidence。
  - **測試**：新增 `rate-limit-wiring.test.ts`（4 測：行為面驗兩條端點被限流／共用同一桶／429 在 parse 前／exact-path 不誤傷 `:id` 子路徑與 GET；接線面鎖 index.ts 名單順序與「全 src 只有一個 limiter 實例」）、`checklist-metering.test.ts`（1 測：生成事件的 userId／orgId／meetingId／idemKey）；`checklist.test.ts` +5 測（冷卻期常數單一真相、期內被跳過、期滿放行、`'slide'`＋手動 check 不受影響、只影響被 uncheck 的那一項）；`checklist-repo.test.ts` +2 測（同來源重勾 `covered_at` 不動、首次/skipped→covered 仍寫入 now）。已做突變驗證：關掉 B 的過濾 → 4 測紅；把 D 的 CASE 比對值換成不可能命中的值 → 該測紅。
- **為什麼**: 對抗式復驗指出的四個缺陷。A 是本 repo 最貴端點全裸（合法登入者可用 HTTP 速度堆 45s in-flight Gemini 呼叫），且「另開一桶」同時造成額度加倍與白 parse；B 使 `uncheck` 在會中實質無效（5 秒後被同一段窗內逐字稿推翻，報告者只能改用語意錯誤的 `skip`）；C 讓 admin 成本明細一半掛在「未知」；D 讓已過期的逐字殘留可因重勾而延後一輪 purge。

### 2026-07-30 12:35 | 對抗式復驗二修：evidence TTL 繞過路徑（隱私洞，含被寫成回歸鎖定的錯誤斷言）＋draft-objective 漏記帳
- **工作區**: packages/crm, apps/server
- **類型**: fix
- **檔案**: `packages/crm/src/repos-checklist.ts`, `packages/crm/src/ports.ts`（僅註解）, `packages/crm/test/checklist-repo.test.ts`, `apps/server/src/realtime/transcript-retention.ts`, `apps/server/src/realtime/transcript-retention.test.ts`, `apps/server/src/realtime/hub.ts`, `apps/server/src/realtime/meetings-routes.ts`, `apps/server/src/realtime/draft-objective-metering.test.ts`（新）
- **改了什麼**:
  - **修正 A①（源頭）`repos-checklist.ts` setStatus 的 covered 分支**：covered_by 換人時一併清 evidence。
    Before `SET status = ?, covered_by = ?, covered_at = ?, updated_at = ?`；
    After 多一句 `evidence = CASE WHEN covered_by = ? THEN evidence ELSE NULL END`（SET 運算式讀**舊列值**，SQLite/pg 皆然；參數多帶一個 `coveredBy`）。同來源重複勾選（manual→manual）保留 evidence，跨來源（transcript/slide/NULL→新來源）一律清。`'pending'`／`'skipped'` 分支語意完全不動。
  - **修正 A②（縱深防禦）`transcript-retention.ts` purge 條件**：Before `WHERE covered_by = 'transcript'`；After `WHERE (covered_by IS NULL OR covered_by <> 'slide')`（**不用 `IS DISTINCT FROM`**——舊版 SQLite 不支援）。年齡運算式、`COALESCE(covered_at, created_at)`、只清 evidence 不刪列，全部保持上一輪做法不變。
  - **測試**：`transcript-retention.test.ts` 把錯誤斷言 `expect(at(2).evidence).toBe("報告者自己的備註")`（假設 manual evidence 恆 NULL）改為 `expect(at(2).evidence).toBeUndefined()` ＋全表 `evidence IS NOT NULL` 只剩 slide 那列；另新增「原始攻擊時序」一測（markCovered transcript → setStatus covered/manual → 斷言 evidence **當下**即 NULL → 再過期 purge 仍 NULL）。`checklist-repo.test.ts` 新增兩測（換來源清 evidence／同來源保留）。
  - **修正 B `hub.checklistGenDeps()`**：Before 回傳 raw `this.gemini`（→ `POST /api/meetings/draft-objective` 的 LLM 呼叫完全不進 `usage_events`）；After 簽名改 `checklistGenDeps(orgId, userId?)`，有 `this.meter` 時包 `meteredGeminiClient({ orgId, kind:"gemini_text", userId, idemPrefix: \`objective:${randomUUID()}\` })`——沿用 `runChecklistGeneration` 既有機制與 kind；`idemPrefix` 帶 per-call uuid（此端點可反覆觸發，用 `checklist:${meetingId}` 那種固定前綴會被冪等鍵去重而少計）；`meetingId` 省略（草擬發生在建會之前）。`meetings-routes.ts` 呼叫端改 `hub.checklistGenDeps(orgId, userId)`（兩者皆取自 JWT）。新測試 `draft-objective-metering.test.ts` 走真 router＋真 authRequired＋假 Gemini/假 Meter，斷言每次落一筆 `gemini_text`、orgId/userId 正確、兩次 idemKey 不同、跨 org 歸屬正確、資料不足時不打 LLM 也不記帳。
- **為什麼**:
  - **A**：`setStatus('covered', by)` 會改 `covered_by` 但（舊實作）不動 `evidence`。真實時序是對話勾稽先 `markCovered(...,'transcript',<逐字片段>)` 寫入 evidence，但 snapshot 廣播有 300ms debounce（`CHECKLIST_BROADCAST_DEBOUNCE_MS`）＋RTT，這段時間 HUD 上該項仍顯示 pending → 報告者點 checkbox → `covered_by` 變 `'manual'` 而 evidence 原封不動 → 該列從此永遠不符 `covered_by='transcript'` 的 purge 條件 → **逐字稿內容永久留存，繞過 M5 §A TTL**。更糟的是上一輪新測試的第 81 行正好斷言「manual 的 evidence 不會被清」，把洞寫成了回歸鎖定。`'slide'` 的 evidence（「第 N 頁」）是唯一可證明非逐字內容者，故改成「只排除 slide」；純手動勾選的 evidence 本來就是 NULL，清它是 no-op。HUD 不渲染 evidence，故此修法零 UX 影響。
  - **B**：同檔案的清單生成路徑本來就有包 metered client（`hub.ts` runChecklistGeneration），draft-objective 卻用 raw client → 這是**漏包**而非設計取捨，違反 ADMIN_CONTRACT §3「記帳補洞——讓 token 花費真實可見」與本檔既有「LLM 呼叫必記帳」約定；漏的是使用者可反覆觸發的端點，admin usage/overview 的 costUsd 因此少計且無法歸屬 org。

### 2026-07-30 12:05 | code-review 三修：slideIdx 頁碼守衛（真 bug）＋checklist evidence 納入 TTL purge＋進度分母排除 skipped
- **工作區**: apps/server, apps/web
- **類型**: fix
- **檔案**: `apps/server/src/generation/checklist-gen.ts`, `apps/server/src/generation/checklist-sanitize.test.ts`（新）, `apps/server/src/realtime/transcript-retention.ts`, `apps/server/src/realtime/transcript-retention.test.ts`（新）, `apps/web/components/hud/ChecklistPanel.tsx`, `apps/web/messages/zh-TW.json`, `apps/web/messages/en.json`
- **改了什麼**:
  1. **修正 1（真 bug）`checklist-gen.ts`**：`sanitizeChecklist` 的 slideIdx 守衛把「大綱列數」當「頁碼上限」。`buildDeckOutline` 預設 `keepEmptyPages:false`＝**跳過沒有文字的頁但保留原始頁碼**（`deck-outline.test.ts:97-101`：5 頁 deck → idx `[0,1,3,4]`、length=4），而 `formatDeckOutline` 輸出 `#${idx}`、`CHECKLIST_SYSTEM:118` 明令模型「填該頁的 #編號」→ 模型回的是**原始頁碼**。
     - Before: `sanitizeChecklist(raw, input.deckOutline.length)`；守衛 `… && Number.isInteger(rawIdx) && rawIdx >= 0 && rawIdx < deckPageCount`
     - After: `sanitizeChecklist(raw, input.deckOutline)`；函式簽名改吃 `readonly DeckOutlineRow[]`，首行 `const pages = new Set(deckOutline.map((r) => r.idx));`，守衛 `category === "talk" && typeof rawIdx === "number" && pages.has(rawIdx)`（`pages.has` 已隱含整數且 >=0）
     - 影響面：修正前模型正確回的 `slideIdx:4` 被判 `4 < 4`=false → 靜默變 undefined → `repos-checklist.ts:114` 落 `slide_idx = NULL` → `hub.ts:512` 翻頁勾稽（`it.slideIdx === prevIndex`）與 HUD `ChecklistPanel.tsx` 的「正在講」高亮對該項**永久失效且無任何 log**；反向地，改用集合檢查後還能**精確排除被跳過的空頁頁碼**（舊版反而放行 idx=2，會把 talk 項綁到純圖頁埋誤劃地雷）。`:211-216` 的函式註解同步改寫（「在 deck 頁數範圍內」→「在大綱實際存在的頁碼集合內；idx 是原始頁序，跳空頁後不連號，不可用列數當上限」）。
     - **新測試** `checklist-sanitize.test.ts`（6 測，`sanitizeChecklist` 原本全 repo 零測試）：以 `buildDeckOutline` 對「5 頁、idx=2 為無 alt 的 image-full」產出 idx=`[0,1,3,4]` 當輸入，鎖住 `slideIdx:4` **保留**／`slideIdx:2`（被跳過的空頁）丟棄／`5`、`-1`、`1.5`、`"3"` 丟棄／`ask`＋`address` 恆 undefined（契約 §6.2）／大綱為空時項目仍保留但 slideIdx 丟棄。
  2. **修正 2（隱私保留缺口）`transcript-retention.ts`**：`meeting_checklist_items.evidence` 在 `covered_by='transcript'` 時存的是**逐字稿位元組前綴**（`hub.ts:405` `runtime.lastEvidenceText = route.persist?.text`，與寫進 `meeting_transcript_segments` 的是同一個字串值，`repos-checklist.ts:150` 截 120 字後落庫），但 TTL purge 只 DELETE segments → 逐字片段在第二張表永久留存，與 `docs/M5_CONTRACT.md:13` 不一致。runOnce 內在既有 DELETE 之後**追加一條 UPDATE**：`SET evidence = NULL, updated_at = ?` `WHERE covered_by = 'transcript' AND evidence IS NOT NULL AND COALESCE(covered_at, created_at) < ? - (COALESCE((SELECT m.retention_days FROM meetings m WHERE m.id = meeting_checklist_items.meeting_id), ?) * ?)`——**過期式子逐字沿用同一條**（`COALESCE(retention_days, 30) * DAY_MS`），只把「列的時間」換成 evidence 實際寫入時間 `covered_at`（markCovered 同時寫 covered_by/covered_at/evidence，等於對應 segment 的 created_at；`COALESCE(..., created_at)` 只為防 DB 被手改成 NULL 而永不過期）。**只清 evidence 欄不刪列**（清單是會議產物，會後檢視要看）、**只清 transcript 來源**（`'slide'` 的 evidence 是「第 N 頁」、`'manual'` 契約上恆 NULL，都不是逐字內容）。一律 SQLite 風格 `?`（PgDbPort 邊界轉 `$n`），log 只印筆數不印文字。`runOnce()` 回傳值語意不變（＝被刪 segment 數），evidence 筆數另行 log，interface 註解已註明。
     - **新測試** `transcript-retention.test.ts`（2 測）：過期場次（retention 7 天、列回撥 30 天）的 transcript evidence → NULL 且 status/coveredBy 不動、`'slide'`「第 3 頁」與硬塞值的 `'manual'` 列**不動**、4 列**一列都沒被刪**、pending 列仍 pending；未過期場次（retention NULL → 30 天預設、列回撥 10 天）evidence 完好、其 segment 未被刪；第二次 runOnce 零命中且 `updated_at` 不再變（冪等）。測試一律「先建 handle 讓開機 purge 對空 DB 跑完 → 再塞資料 → 才 runOnce」，否則 fire-and-forget 的開機 purge 會先把資料清掉、讓回傳筆數變 0（第一版就踩到）。
  3. **修正 3（HUD 進度分母）`ChecklistPanel.tsx`**：`skipped`＝報告者主動判定「這場不講」，留在分母裡會讓進度永遠追不到 100%。
     - Before: `const total = items.length;`（分母含 skipped；進度條寬度同源）
     - After: `const skipped = useMemo(…status === "skipped"…)`；`const total = items.length - skipped;`（＝pending＋covered），`pct` 用同一分母；**分母為 0（全部略過）時**進度文案改顯示 `t("allSkipped")`、`.mc-checklist__meter` 整段不渲染（不會 NaN／除零／進度條爆掉），`.mc-checklist__next` 在 `total===0` 時不再誤顯「都講完了」。
     - i18n 新增 `hud.checklist.allSkipped`（zh-TW「全部已略過」／en "All skipped"），兩份同步 → 300/300 → **301/301，key 集合完全相同**。
- **不變量**: **I1/I2/I3 未觸及**——三處都不產生 PatchOp、不碰 approval gate、不改 WS 授權；`ChecklistPanel.tsx` 仍只被 `HudView.tsx` import（`components/present/**` 零改動）。凍結契約 `docs/MEETING_CHECKLIST_CONTRACT.md` **未修改**（修正 1、3 落在契約既有敘述內；修正 2 補的是契約沒涵蓋的 retention 面）。
- **驗證**: `apps/server` `npx tsc --noEmit` EXIT=0、`npx vitest run` 57 檔 337 測全綠（基準 55/329 → +2 檔 +8 測）；`packages/crm` `npx vitest run` 10 檔 76 測全綠（未動 crm 程式碼，故數字持平）；`apps/web` `npx tsc --noEmit` EXIT=0、`npx next build` 成功（19 路由，與基準相同）、i18n parity 301/301。**修正 1 的測試在修正前確實紅**：先寫測試後跑 → `expected undefined to be 4`（`checklist-sanitize.test.ts:43`），修正後 6 測全綠；另 `node -e` 驗算舊守衛 `rawIdx(4) < deckPageCount(4)` → `false`，確認不是測試寫法造成的假紅。
- **為什麼**: `/code-review` 五鏡頭中四個獨立確認修正 1 是真 bug（會靜默廢掉翻頁勾稽與「正在講」高亮）；修正 2 是升級修的隱私／保留政策缺口（逐字稿位元組繞過 TTL）；修正 3 是指揮官先前已列的待修（skipped 不該算在待完成分母裡）。

### 2026-07-30 14:20 | 「會中進行」兩入口改造：改名＋同分頁導覽＋新準備頁＋舞台可用性（web-only，未部署）
- **工作區**: apps/web
- **類型**: feat（含 fix：解掉「簡報舞台」100% 死路入口）
- **檔案**: `apps/web/components/AppShell.tsx`, `apps/web/components/home/HomeDashboard.tsx`, `apps/web/app/[locale]/copilot/page.tsx`, `apps/web/components/copilot/CockpitView.tsx`, `apps/web/components/copilot/CopilotView.tsx`, `apps/web/components/present/PresentStage.tsx`, `apps/web/components/present/PresentStart.tsx`（新）, `apps/web/app/[locale]/present/start/page.tsx`（新）, `apps/web/components/sim/MeetingSimulator.tsx`, `apps/web/components/ui/Markdown.tsx`, `apps/web/messages/zh-TW.json`, `apps/web/messages/en.json`, `apps/web/app/globals.css`, `apps/web/app/studio-present.css`
- **改了什麼**（依 ROM 2026-07-28 19:40 七點）:
  1. **改名＋名稱統一**：`nav.present`「簡報舞台」→「會議簡報」（en `Present stage`→`Meeting Slides`）、`nav.copilot`「會中副駕 · HUD」→「MeetCopilot」（en 同）；`present.title`／`copilot.title`／`home.subtitle`／`home.liveNote` 一併對齊；`CopilotView.tsx:184` 的硬編碼中文「會中副駕 · 擷取端」（三行）改走 i18n `copilot.captureHeading/captureLead/capturePlatform` 並統一為「MeetCopilot · 擷取端」；`MeetingSimulator.tsx:573`、`Markdown.tsx:6` 殘留字串同步。
  2. **移除兩入口 external**：`AppShell.tsx` `NavItem.external` 欄位整個刪除，`isActive(pathname, href, external)`→`isActive(pathname, href)`（原本第一行 `if (external) return false` 讓兩項永遠不高亮），`target="_blank"`／`rel="noopener"`／↗ span／`title={t("nav.newTab")}` 一併移除；`HomeDashboard.tsx` 的 `Surface.external` 與三處同款程式碼同步。`nav.newTab` 鍵已無人使用 → 雙語一併刪除（parity 維持）。
     - Before: `{ key:"present", href:"/present", …, external:true }`（裸 `/present` 無 deckId → `PresentStage` 必落「沒有可播放的簡報」終態＝100% 死路，且先開新分頁才撞牆）
     - After: `{ key:"present", href:"/present/start", … }`（app 內準備頁，同分頁）
  3. **`/copilot` 掛 AppShell**：`copilot/page.tsx` 包 `<AppShell>`；`CockpitView` 移除自帶的 `ToastProvider`（AppShell 已提供，嵌套會多渲染一個 `.mc-toast-viewport`）並把整個 return 去掉一層縮排。CSS 加 `.mc-shell__body .mc-cockpit`（去掉重複 max-width/padding）＋`@media (max-width:1348px)` 讓兩欄 grid 在側欄旁提早堆疊（standalone 的 1100px 斷點沒算到 248px 側欄）。`/present`、`/hud` **維持不掛**（I3）。
  4. **新準備頁 `/present/start`**（掛 AppShell）：`listDecks()` 列 deck（沿用 `.mc-deckgrid`/`.mc-deckcard`＋`StateBoundary` 三態）、預設選最近更新那份、顯示標題／頁數／更新時間，兩個動作＝「單機播放」（`buildStaticPresentUrl`）與「連線會議播放」（`buildPresentUrl`，無 creds 時 `disabled` 並附去 `/copilot` 的指引連結）。導覽一律 `router.push`，**無 `window.open`**；builder 回的是絕對網址（給第二裝置用），故加 `toInAppPath()` 取 pathname+search 交給 locale-aware router。無 deck → `.mc-empty` 空狀態＋去 `/studio` 的 CTA。
  5. **舞台同分頁全螢幕**：`PresentStage` 加 `fs`/`fsOk` 狀態、`toggleFullscreen()`（`documentElement.requestFullscreen` ／ `webkit*` 後援；回傳值一律 `Promise.resolve(p).catch(()=>{})`＋外層 try/catch → 被拒或不支援就靜默降級成普通全視窗，不 throw 不白畫面）、`fullscreenchange`＋`webkitfullscreenchange` 同步狀態、`F`/`f` 鍵切換（keydown 本身即使用者手勢）。**不自動進入全螢幕**。
  6. **舞台補基本可用性**：加 `.mc-present__controls`（‹ › 翻頁＋⛶ 全螢幕，3 顆低調按鈕）＋首次進入的 `.mc-present__hint`（`→ 下一頁 · ← 上一頁 · F 全螢幕`，5 秒後元件卸下）；`go()` 內 `setHint(false)` 讓任何翻頁立刻收提示。控制列顯隱**不用 CSS `:hover`**——`.mc-present` 是 `position:fixed;inset:0` 滿版元素，指標在視窗內 `:hover` 恆為真，「平時不顯眼」不會成立（實測 opacity 一直是 1）→ 改用 `pointermove`/`pointerdown` 喚醒 `is-on`、靜止 2.5 秒淡回 0.16（播放軟體慣例），另保留 `.mc-present__controls:hover`／`:focus-within` 與 `@media (hover:none)` 常駐 0.55。死路終態按鈕 `<Link href="/">{t("backHome")}</Link>` → `<Link href="/present/start">{t("pickDeck")}</Link>`，`emptyDesc` 文案同步（原本叫人「從 App 開啟一份簡報」卻只丟回首頁）。
  7. **假 QR 誠實化**：移除 `CockpitView.tsx:77-87` 的裝飾性假 QR（註解自承 "intentionally NOT a scannable code"）與 `.mc-cockpit__second-qr` 用法；`secondDeviceDesc` 雙語改為複製連結的真實敘述（不再說「掃描 QR」），`secondDeviceQrAlt` → `secondDeviceLinkLabel`。真 QR encoder 仍記為技術債（不為此加依賴）。
- **不變量**: **I3 守住**——`PresentStage.tsx` 的 import 清單**一個都沒加**（全螢幕與翻頁只用瀏覽器原生 API），`/present` 與 `/hud` 維持不掛 AppShell，新頁 `/present/start` 零 HUD import；實機 grep 頁面 HTML 亦無 `mc-hud`/`transcript`/`suggestion`/`info_card`/`signals`/`mc-sidebar` 任何標記。**I1/I2 未觸及**（純導覽與展示層，不碰 deck patch／approval gate／WS 授權）。
- **驗證**: `npx tsc --noEmit` EXIT=0；`npx next build` EXIT=0（路由 18→19，新增 `/[locale]/present/start`）；messages parity 274/274 → 300/300（無單邊鍵）；Playwright headless 實機走查 15 張截圖全綠、console 0 error（後端以 route mock）。
- **為什麼**: 使用者看側欄截圖說「點進去的 UI 互動太不直覺」。偵察證實是硬缺陷而非主觀：側欄「簡報舞台」是 100% 死路（裸 `/present` 無 deckId）、present 全庫 0 個 fullscreen 呼叫與 0 個滑鼠可操作元素、`/copilot` 新分頁內 0 個回 App 連結、同一功能有三種叫法、還有一個假 QR。使用者三點指示：改名、重新設計互動、只有 MeetCopilot 需要新分頁且不是立刻開。

### 2026-07-30 10:35 | 會中待講清單 WP-C：web 前端 HUD 面板＋建會表單三欄（未部署）
- **工作區**: apps/web
- **類型**: feat
- **檔案**: `apps/web/components/hud/ChecklistPanel.tsx`（新）, `apps/web/components/hud/HudView.tsx`, `apps/web/components/copilot/CopilotView.tsx`, `apps/web/lib/api.ts`, `apps/web/messages/zh-TW.json`, `apps/web/messages/en.json`, `apps/web/app/globals.css`
- **改了什麼**:
  - **新 `ChecklistPanel.tsx`**（契約 §8）：收合態單行（進度「已講 n/總」＋56px 進度條＋下一個待辦截斷＋展開鈕），CSS 硬保險 `max-height:48px`；展開態依 `CHECKLIST_CATEGORIES` 分必講／必問／必回應三組，每列 checkbox＋title（covered→刪除線＋淡化）＋`slideIdx===currentSlideIdx` 的「正在講」標記＋`nice` 加分標＋pending 的「略過」／skipped 的「復原」。`status:"generating"`→spinner＋「清單生成中…」；`"failed"`→短提示；`status===null`→`return null`（不佔版面）；ready＋零項目→`.mc-hud__empty` 空狀態。wire 型別用 `Extract<ServerMessage,{type:"checklist"}>` 推導，避免與凍結契約漂移。
  - **`HudView.tsx`**：加 `checklist`/`checklistStatus`/`currentSlideIdx` state（`:60-63`）；`onMessage` 新增 `case "checklist"`（`:127-134`）——**replace 語意**（`setChecklist(msg.items)` 整份換掉，非 append；本檔唯一的覆寫型 reducer，已在註解標明別照抄成 append），`msg.currentSlideIdx` 為 undefined 時保留舊值不清高亮；`case "session_state"` 原本丟掉 `msg.committedIndex`，改為 seed `currentSlideIdx`（同源 runtime 高水位）。新增 `onChecklistAction`（`:170-179`）只送 `{type:"checklist_action",itemId,action}`，**刻意無樂觀更新**。版面順序 Before: `SuggestionQueue → TranscriptStream → InfoCardStream → DeepResearchBox`；After: `ChecklistPanel →（同上）`。
  - **`CopilotView.tsx` SetupPanel**（契約 §9）：加「簡報（選填）」「對方公司（選填）」兩個 `<select>`（來源＝既有 `listDecks()`／`listCompanies()`，載失敗/空→該欄不渲染，絕不擋建會）＋`<details class="mc-cap__objective">` 內的「會議目標」單行。選了 deck 或 company → 打 `draftMeetingObjective` 自動預填（`objectiveEdited` latch：使用者動過就不再覆寫；title 走 ref 不當 effect dep，避免每敲一字打一次 API；失敗只吞掉）。送出 Before: `createMeeting({ title })`；After: `createMeeting({ title, deckId||undefined, companyId||undefined, objective||undefined })`——三欄全空時 body 逐字等於改動前（實測 `{"title":"..."}`）。
  - **`lib/api.ts`**：`createMeeting` input 加 `objective?`；新增 `draftMeetingObjective({deckId?,companyId?,title?}) → {objective}`（契約 §6.1，回空字串＝沒建議不是失敗）。
  - **`globals.css`**：新增 `.mc-checklist*` 一段（沿用 `.mc-hud__panel` 形狀＋`--mc-accent`/`--mc-border`/`--mc-field` 等既有 token，零硬編碼色碼）＋`.mc-cap__objective > summary`。
  - **i18n**：`hud.checklist.*` 19 鍵＋`copilot.*` 8 鍵，zh-TW/en 各 248→274，parity 相等。
- **為什麼**: MEETING_CHECKLIST_CONTRACT §8/§9 的消費端。**I3**：面板只掛在 HUD 樹，`components/present/**` 零改動、全庫 grep `ChecklistPanel` 僅 `HudView.tsx` 一處 import。**I2**：勾選只是 WS 訊息，授權在 server；前端不樂觀更新——實測掐斷 `WebSocket.send` 後點 checkbox，該列 `covered/checked` 皆維持 false（只有 `is-inflight` 淡化回饋），證明真相來源是 server 回的全量 snapshot。收合態實測 48.0px，`建議批准佇列` 位於 top=76/bottom=161（430×860 viewport），沒被擠出首屏。

### 2026-07-30 10:05 | 會中待講清單 WP-B：server 端生成＋三路勾稽（未部署）
- **工作區**: apps/server
- **類型**: feat
- **檔案**: `apps/server/src/generation/checklist-gen.ts`（新）, `apps/server/src/generation/slide-gen.ts`, `apps/server/src/generation/deck-outline.test.ts`（新）, `apps/server/src/analysis/analysis-engine.ts`, `apps/server/src/analysis/gemini-analysis.ts`, `apps/server/src/realtime/hub.ts`, `apps/server/src/realtime/ws-server.ts`, `apps/server/src/realtime/session-runtime.ts`, `apps/server/src/realtime/meetings-routes.ts`, `apps/server/src/realtime/meeting-store.ts`, `apps/server/src/realtime/checklist.test.ts`（新）
- **改了什麼**（一律以 `docs/MEETING_CHECKLIST_CONTRACT.md` §5–§7 為準）:
  - **§6.4 outline 抽出**：`slide-gen.ts` 新增 export `buildDeckOutline` / `capDeckOutlineTotal` / `formatDeckOutline` ＋ `REVISE_OUTLINE_OPTIONS` / `DECK_OUTLINE_TOTAL_MAX_CHARS`。Before＝`reviseSlides` 內嵌 `slides.map((s,i)=>\`#${i} [${s.template}] ${extractSlideText(s)…slice(0,70)}\`).join("\n")`；After＝`formatDeckOutline(buildDeckOutline(slides.map(spec=>({spec})), REVISE_OUTLINE_OPTIONS))`，**逐字等價**由 `deck-outline.test.ts` 以 golden 公式鎖住。逐頁文字序＝`extractSlideText(spec)`→空則 `textExtract`→仍空跳過；整份上限 12,000 字等比截斷。
  - **§6.2 生成**：`checklist-gen.ts` 新增 `generateChecklist`（`config.gemini.extractModel`＋responseSchema＋deadline 45s＋attempts 2＋MAX_TOKENS 把 outline 砍半重試一次）、`sanitizeChecklist`（枚舉/24 字 title/keywords≤5/`slideIdx` 僅 talk 且需在頁數內/上限 14 且保住三類/**0 起連號 idx**）、`draftMeetingObjective`（§6.1）、`gatherChecklistContext`（org-scoped 讀 deck＋company＋contacts＋deal，逐筆 best-effort）。
  - **§6.1 端點**：`POST /api/meetings/draft-objective`（auth＋org scope＋`TokenBucketRateLimiter` 限流；無 deck 也無 company／任何失敗 → `{objective:""}` 不 throw）。
  - **§6.3 建會**：`meeting-store.ts` INSERT 補 `deck_id`/`objective`（SELECT/`toRef` 一併帶出）；`meetings-routes.ts` 收 `objective` 並在建會成功後同步呼叫 `hub.startChecklistGeneration()`（內部 fire-and-forget、每場一次、失敗只廣播 `failed`）。
  - **§7.1 對話勾稽（零額外 LLM 呼叫）**：`analysis-engine.ts` 新增 `AnalysisResult`/`PendingChecklistHint` 與 optional `setPendingChecklist`，`onSignals` 回呼加第二參數；`gemini-analysis.ts` schema 加 `coveredItemIds`、pending 非空才附加 prompt 一節、sanitize 丟棄不在 pending 集合的 id。窗口 10 段/90 秒/節流 5 秒/單飛鎖**原封不動**。
  - **§7.2 翻頁勾稽**：`session-runtime.ts` 加 `lastCommitAt`＋`lastEvidenceText`；`ws-server.ts` 在既有 I2 gate 內、`committedIndex` 前進前呼叫 `hub.onPageCommitted(runtime, prevIndex)`；停留 ≥`SLIDE_DWELL_COVER_MS` 才 `markCovered(...,'slide',"第 N 頁")`。
  - **§5/§7.3 手動**：`ws-server.ts` 新增 `checklist_action` case，presenter-only（形狀同 `suggestion_action`），非 presenter 回 `error{code:"forbidden_not_presenter"}`。
  - **§7.4 廣播**：hub 全量 snapshot **只走 `broadcast(..., "hud")`**（I3）、無改變不廣播、300ms debounce、`currentSlideIdx`＝`committedIndex`（-1 則省略）；hud 一連上補發 snapshot；`disposeSession`/`disposeAll` 清 timer 與階段狀態。
  - `evidence` 只取 `route.persist?.text`（已 redact 且本場已同意持久化）→ ephemeral 場次恆 NULL，避免 checklist 反向持久化逐字稿（M5 §A）。
  - `ws-server.ts` 的 `hub.onPageCommitted?.()` 用 optional call：既有 `ws-presenter-authz.test.ts` 的精簡 hub 替身沒有這個方法，I2 身分閘不該因附加的勾稽副作用而炸。
- **為什麼**: 會前依「目標＋簡報全文＋CRM 情報」生成必講/必問/必回應清單，會中隨對話與翻頁自動劃掉，HUD 永遠看得到「還剩什麼沒講」（WP-B＝server 端閉環；WP-A 已備地基，HUD 前端與建會表單為後續工作包）。驗收：`apps/server` `tsc --noEmit` EXIT=0、vitest 55 檔 329 測全綠（基準 53/306）。

### 2026-07-28 17:07 | 會中待講清單 WP-A：migration 023 雙份＋shared 型別/wire 協定＋ChecklistRepository（未部署）
- **工作區**: packages/shared, packages/crm
- **類型**: feat
- **檔案**: `packages/crm/migrations/023_meeting_checklist.sql`, `packages/crm/migrations-pg/023_meeting_checklist.sql`, `packages/shared/src/checklist.ts`, `packages/shared/src/index.ts`, `packages/shared/src/protocol.ts`, `packages/crm/src/repos-checklist.ts`, `packages/crm/src/ports.ts`, `packages/crm/src/core.ts`, `packages/crm/test/checklist-repo.test.ts`, `docs/API_CONTRACT.md`
- **改了什麼**:
  - **migration 023（SQLite＋PG 雙份，語意等價）**: `meetings` 加 `deck_id`／`objective`（修既有債：deck 綁定原本只在記憶體、會議目標無處存；**刻意不改用既有 `agenda` 欄**）；`deck_slides` 加 `text_extract`（C2 預留，C1 不寫入）；新表 `meeting_checklist_items`（16 欄＋4 條 CHECK：category/priority/status/covered_by）＋索引 `idx_meeting_checklist_org_meeting(org_id, meeting_id, idx)`。SQLite 一欄一條 `ALTER TABLE ADD COLUMN`（三欄皆 nullable，不需 DEFAULT 回填）、時間用 INTEGER；PG 用 `ADD COLUMN IF NOT EXISTS`、時間/序號用 BIGINT（照 018 慣例）。
  - **`packages/shared/src/checklist.ts`（新）**: 逐字實作契約 §4——`CHECKLIST_CATEGORIES`/`CHECKLIST_STATUSES`/`CHECKLIST_COVER_SOURCES` 三組 const tuple＋衍生型別、`ChecklistItem`、`NewChecklistItem`（Omit id/status/coveredBy/coveredAt/evidence）、四個常數（`CHECKLIST_MAX_ITEMS=14`／`MIN=6`／`PROMPT_MAX_PENDING=15`／`SLIDE_DWELL_COVER_MS=20_000`）。`index.ts` 加一行 re-export。
  - **`protocol.ts`（wire §5）**: `ClientMessage` 加 `{type:'checklist_action', itemId, action:'check'|'uncheck'|'skip'}`（presenter-only）；`ServerMessage` 加 `{type:'checklist', status:'generating'|'ready'|'failed', items:ChecklistItem[], currentSlideIdx?}`（**hud-only、全量 snapshot、replace 語意**）。`docs/API_CONTRACT.md` §6 同步兩型別＋把 `checklist_action` 併入 presenter-only 授權句（protocol.ts 明文要求兩者不得漂移）。
  - **`repos-checklist.ts`（新，`SqliteChecklistRepository`，port-agnostic 兩驅動共用）**: `replaceAll`（tx 內先 DELETE 後逐筆 INSERT，回讀依 idx 排序）／`list`／`markCovered`／`setStatus`。**`markCovered` 只更新 `status='pending'` 的列**——先 SELECT 命中的 pending id 集合、再據此 UPDATE＋回讀，**回傳「這次真的被改動」的項目，零變化回空陣列**（呼叫端據此不廣播，契約 §7.4）；`evidence` 落庫前截斷 120 字。`setStatus`：`covered`→`covered_by=by??'manual'`＋`covered_at=now`（evidence 不動）；`pending`／`skipped`→清空 covered_by/covered_at/evidence。**每個方法 orgId 都進 WHERE**，跨 org 一律回空/null 不 throw。`parseKeywords` 對爛資料三層降級（非 JSON→`[]`、非陣列→`[]`、陣列內非字串元素過濾）。
  - **`ports.ts`／`core.ts`**: 新增 `ChecklistRepository` 介面（契約 §3 簽名逐字）＋掛進 `CrmCore.checklist`；`assemble()` 註冊 `new SqliteChecklistRepository(port)`。
  - **`test/checklist-repo.test.ts`（新，6 測）**: replaceAll→list 往返（含整份取代非累加）、markCovered 只動 pending（manual covered 不被 transcript 覆寫）、無變化回空＋重跑冪等＋skipped 不被覆寫、setStatus 三態、org 隔離零副作用、keywords_json 爛資料韌性。
- **為什麼**: 會中待講清單新產品線（ROM 2026-07-28 16:54 四項岔路拍板、契約凍結於 `docs/MEETING_CHECKLIST_CONTRACT.md`）的工作包 A＝資料層＋契約型別，供後續 WP（生成端／勾稽端／HUD）在凍結接縫上平行開發。**不變量**：I1 不觸及（不產生任何 PatchOp、不碰 `spec_json`）；I2 由 wire 型別註記 presenter-only（閘實作在 WP-B 的 ws-server）；I3 由 `checklist` 訊息的 hud-only 註記＋API_CONTRACT 明文守住。
- **驗證**: `packages/shared`／`packages/crm` `tsc --noEmit` 皆 EXIT 0；**根 `npm run typecheck`（含 apps/server／web／admin）EXIT 0**（新增 union 成員未打破任何 exhaustive switch）；`packages/crm` vitest **76 passed / 10 files**（基準 70＋新 6，零退步）；`npm run build` 兩包 EXIT 0（dist 產出含 `repos-checklist.js`）。**實跑 migration**：對真實檔案型 SQLite 跑 001→023（`MAX(version)=23, count=23`），驗證三處新欄/新表/索引落地、CHECK 對違規 category/covered_by 生效、合法列可插入；同一 DB 再跑一次 runner 仍 `v=23,n=23`、資料不動（冪等）。PG 版未實跑（本機無 Postgres 實例）。未 commit／未部署（硬規則 10）。
