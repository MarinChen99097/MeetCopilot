-- 003_prospect.sql — 對方公司英雄表 companies ＋ 可爬子表（CRM_SCHEMA §4）。
-- §0：無 SQL FOREIGN KEY；enum＝TEXT + CHECK(col IN(...))；org_id 全帶；時間 epoch-ms；JSON 存 TEXT。
-- UNIQUE(org_id, domain) 給 upsertFromCrawl 的 dedupe 硬保證（SQLite NULL 互不相等，無 domain 列不受限）。
...
CREATE TABLE IF NOT EXISTS company_locations (
  ...
  city        TEXT,                                    -- C
  address     TEXT,                                    -- C
  is_primary  INTEGER DEFAULT 0,                       -- C  0/1
  created_at  INTEGER NOT NULL,                        -- S
...
CREATE TABLE IF NOT EXISTS company_news (
  ...
  url          TEXT,                                   -- C
  source       TEXT,                                   -- C
  published_at INTEGER,                                -- C
  summary      TEXT,                                   -- C  embedded for RAG
  category     TEXT,                                   -- C  見 CHECK
  sentiment    REAL,                                   -- C
  relevance    REAL,                                   -- C
  embedded     INTEGER DEFAULT 0,                      -- S  0/1 已嵌入
  created_at   INTEGER NOT NULL,                       -- S
...
CREATE TABLE IF NOT EXISTS company_tech (
  ...
  first_seen_at INTEGER,                               -- C
  last_seen_at  INTEGER,                               -- C
  created_at    INTEGER NOT NULL                       -- S
);
