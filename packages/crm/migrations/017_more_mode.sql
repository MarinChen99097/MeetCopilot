-- 017_more_mode.sql — 放寬 crawl_jobs.mode 的 CHECK 以容納 'more'（「研究更多」補缺升級模式；RESEARCH_UPGRADE v2）。
-- 依 §0 慣例：enum＝TEXT + CHECK(col IN(...))。SQLite 無法 ALTER CHECK，故沿用 010 的「建新表→複製→換名」重建。
-- 欄位/索引與 010 重建後的 crawl_jobs 一字不差（006/010 後無他 migration 動過本表），僅 mode 的 CHECK 多加 'more'。
-- 重建保留既有 quick/detailed/deep job 列（研究執行簿記，非業務資料；重建只是換 CHECK 定義）。
...
CREATE TABLE IF NOT EXISTS crawl_jobs_new (
  ...
  CHECK (mode IN ('quick','detailed','deep','more')),
