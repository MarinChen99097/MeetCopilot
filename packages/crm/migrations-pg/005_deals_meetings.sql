-- 005_deals_meetings.sql (Postgres) — 商機、會議、逐字稿、訊號、筆記、活動（CRM_SCHEMA §6-8）。SQLite migrations/005 的 pg 方言對映。
...
CREATE TABLE IF NOT EXISTS deals (
  ...
  amount                    DOUBLE PRECISION,         -- H
  currency                  TEXT,                     -- H
  probability               BIGINT,                   -- H  0-100
  forecast_category         TEXT,                     -- H
  deal_type                 TEXT,                     -- H  new/expansion/renewal
  expected_close_at         BIGINT,                   -- H
  actual_close_at           BIGINT,                   -- H
...
CREATE TABLE IF NOT EXISTS meetings (
  ...
  meeting_type       TEXT,                            -- H
  platform           TEXT,                            -- S/H
  scheduled_at       BIGINT,                          -- S/H
  started_at         BIGINT,                          -- S
  ended_at           BIGINT,                          -- S
  duration_sec       BIGINT,                          -- S
  status             TEXT,                            -- S
  presenter_user_id  TEXT,                            -- S/H FK users
  agenda             TEXT,                            -- H
  summary            TEXT,                            -- S  embedded for RAG
  outcome            TEXT,                            -- S/H
  sentiment          DOUBLE PRECISION,                -- S
...
CREATE TABLE IF NOT EXISTS meeting_attendees (
  ...
  name           TEXT,                                -- S/H
  is_internal    BIGINT,                              -- S/H 0/1
  role_in_meeting TEXT,                               -- S/H
  talk_time_sec  BIGINT,                              -- S
  sentiment      DOUBLE PRECISION,                    -- S
  created_at     BIGINT NOT NULL                      -- S
);
