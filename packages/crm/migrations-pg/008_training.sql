-- 008_training.sql (Postgres) — 語音模擬訓練：training_sessions / training_reports（M234_CONTRACT §M4；API_CONTRACT §7）。SQLite migrations/008 的 pg 方言對映。
-- §0/§10：無 SQL FOREIGN KEY；enum＝TEXT + CHECK；org_id 全帶；時間 epoch-ms（bigint）；JSON 存 text。
-- 型別對映：INTEGER → bigint；TEXT/_json → text。
