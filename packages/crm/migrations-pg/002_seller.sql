-- 002_seller.sql (Postgres) — 賣方側（rep 自家公司/產品/競品；CRM_SCHEMA §3）。SQLite migrations/002 的 pg 方言對映。
-- §0/§10：無 SQL FOREIGN KEY；enum＝TEXT + CHECK；時間 epoch-ms（bigint）；布林 0/1（bigint，repo 存原始 0/1）；JSON 存 text。
