...
async function extractPage(
  ...
  // 圖片來源：og/twitter meta + 頁內 <img> + **CSS background-image**（inline style + <style> 塊 url(...)，每頁 ≤10）。
  // bg 圖以 {src,alt:'',w:0,h:0} 併入 images（w/h=0 不觸發追蹤像素啟發式）；下游 sanitizeCrawledImages 同既有過濾＋去重＋每頁上限。
  const rawImg = await browserPage
    .evaluate(
      "(function(){function abs(u){if(!u)return null;try{return new URL(u,document.baseURI).href;}catch(e){return null;}}var og=null;var sels=['meta[property=\"og:image\"]','meta[property=\"og:image:secure_url\"]','meta[property=\"og:image:url\"]','meta[name=\"twitter:image\"]','meta[name=\"twitter:image:src\"]'];for(var i=0;i<sels.length;i++){var m=document.querySelector(sels[i]);if(m){var a=abs(m.getAttribute('content'));if(a){og=a;break;}}}var nodes=Array.prototype.slice.call(document.querySelectorAll('img'));var images=[];for(var j=0;j<nodes.length&&images.length<60;j++){var el=nodes[j];var s=el.currentSrc||el.getAttribute('src')||el.src||'';var a2=abs(s);if(!a2)continue;images.push({src:a2,alt:el.getAttribute('alt')||'',w:el.naturalWidth||el.width||0,h:el.naturalHeight||el.height||0});}var bg=[];var bgSeen={};var URL_RE=/url\\((['\"]?)([^'\")]+)\\1\\)/g;function pushBg(u){var a3=abs(u);if(!a3||bgSeen[a3])return;bgSeen[a3]=1;bg.push({src:a3,alt:'',w:0,h:0});}var styled=Array.prototype.slice.call(document.querySelectorAll('[style]'));for(var k=0;k<styled.length&&bg.length<10;k++){var bi=styled[k].style?styled[k].style.backgroundImage:'';if(bi&&bi.indexOf('url(')>=0){var mm;URL_RE.lastIndex=0;while((mm=URL_RE.exec(bi))!==null&&bg.length<10){pushBg(mm[2]);}}}var styleTags=Array.prototype.slice.call(document.querySelectorAll('style'));for(var s2=0;s2<styleTags.length&&bg.length<10;s2++){var tc=styleTags[s2].textContent||'';if(tc.indexOf('url(')<0)continue;var m2;URL_RE.lastIndex=0;while((m2=URL_RE.exec(tc))!==null&&bg.length<10){pushBg(m2[2]);}}return{ogImage:og,images:images.concat(bg)};})()",
    )
    .catch(() => null);
