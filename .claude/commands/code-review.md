...
3. **信心評分（只留 ≥80）**：對每個 finding 另派 opus agent 對抗式驗證（預設立場＝「這是誤報」，被證據說服才給高分），rubric **逐字**給驗證 agent（2026-07-09 自 ezpagesite `/code-review` 步驟 5 移植）：
   - **0**＝完全不可信：經不起檢視的誤報，或是既有（非本批改動）問題。
   - **25**＝略有可信：可能是真問題但也可能誤報，無法驗證為真；風格問題且制度檔未明確要求也算此級。
   - **50**＝中度可信：驗證了是真問題，但屬 nitpick 或實務罕見；相對整批不重要。
   - **75**＝高度可信：複查過、很可能實務會踩、現行寫法不足；或直接違反 I1/I2/I3/SSRF/A1–A3/凍結契約。
   - **100**＝絕對確定：複查確認一定是真問題且會頻繁發生，證據直接支持。
   <80 濾掉；50–79 可另列「borderline」清單供指揮官參考，不進正式結果。
